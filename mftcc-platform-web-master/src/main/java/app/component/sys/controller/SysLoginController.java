...
public class SysLoginController extends BaseFormBean {
    ...
    public String shiroCheck(Model model, String opNo, String opName, String passwordhash, String frame, String ztName) throws Exception {
        //输入 /sysLogin/userLogin 不重新登陆
        String userNo = User.getRegNo(this.request);
        if(StringUtils.isNotEmpty(userNo) && !"null".equals(userNo)){
        ...
        if (sysUser != null) {
            ...
            if("0".equals(isUse) && BizPubParm.OP_NO_TYPE4.equals(opNoType)){
                this.addActionError(model, "该用户不存在,无法登陆！");
                return exitStr;
            }
            //核心企业人员不能登录
            if("0".equals(isUse) && BizPubParm.OP_NO_TYPE5.equals(opNoType)){
                this.addActionError(model, "该用户不存在,无法登陆！");
                return exitStr;
            }
            ...
            if (password.equals(sysUser.getPasswordhash())) {
                ...
                if (sysUser.getOpSts() == null || "0".equals(sysUser.getOpSts())) {
                    //？？防止密码验证通过后不能登陆把密码带回页面
                    sysUser.setPasswordhash(null);
                    this.addActionError(model, "该用户已失效,无法登陆!");
                    return exitStr;
                } else if (sysUser.getOpSts() == null || "2".equals(sysUser.getOpSts())) {
                    //？？防止密码验证通过后不能登陆把密码带回页面
                    sysUser.setPasswordhash(null);
...
//		}
    ...
    private String handleLoginSuccessBusiness(Model model, String remeg, SysUser sysUser, String returnflag, String frame, String opNo, HttpServletRequest request) throws Exception {
        ...
        if ("viewIE8".equals(frame)) {
            ...
            returnFlag = viewMode(sysUser, request,model);
        ...
        if ("main".equals(remeg))//XL安全审计调用接口，记录登录信息
        {
            ...
            } else if ("toup".equals(remeg.substring(0, 4))) {
        ...
        Map<String,String> viewMenuTypeMap =  cu.getMapByKeyName("VIEW_MENU_TYPE");
        if(viewMenuTypeMap.containsKey("0")){
            model.addAttribute("viewMenuType","0");
        }else{
            model.addAttribute("viewMenuType","1");
     ...
     */
    private String viewMode(SysUser sysUser, HttpServletRequest request,Model model) throws Exception {
