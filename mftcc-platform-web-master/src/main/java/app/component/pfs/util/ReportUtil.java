...
public class ReportUtil {
	     ...
	     try { 
	         ...
	         while (pattern.matcher(string).find()) { 
	             ...
	             while (matcher.find()) { 
	                 ...
	                 string = operationReplace(string);
	             } 
	 ...
	 private static String computeStirngNoBracket(String string) { 
	     ...
	     String regexMultiAndDivision = "[\\d\\.]+(\\*|\\/)(\\+|\\-){0,1}[\\d\\.]+"; 
	     String regexAdditionAndSubtraction = "(^\\-)?[\\d\\.]+(\\+|\\-)(\\+|\\-){0,1}[\\d\\.]+";
	     String temp = ""; 
	     int index = -1; 
	     // 解析乘除法 
	  ...
	  * @return
	  */
	 public static String operationReplace(String string){
		 if (string.indexOf("+-")!=-1) { 
	    	 string = string.replaceAll("\\+-", "-"); 
	     }
	     if (string.indexOf("--")!=-1){ 
	 ...
	 public static void main(String[] args) { 
	     String s = "1.0*-(-8.2+0.0)*2-(1.1-1)"; 
	     s = computeString(s); 
	     DecimalFormat    df   = new DecimalFormat("######0.00");  
	     System.out.println(df.format(Double.parseDouble(s))); 
	 } 
