    ...
    if (typeof (aloneFlag) != "undefined") {
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                var zTreeObj = $.fn.zTree.init($("#uploadTree"), setting_upload,
                    zTreeNodesDoc);
            },
            error : function() {
        ...
        url = webPath + "/docManage/getCusDocNodesAjax?" + docParm;
        $.ajax({
            url : url,
            type : "POST",
            dataType : "json",
        ...
        $.ajax({
            ...
            success : function(data) {
                query = data.query;
                if(data.zTreeNodes.length>0){
                    $("#buttonDoc").css("display","inline-block");
                }
                if (typeof (aloneFlag) != "undefined") {
                    zTreeNodesDoc = JSON.parse(data.zTreeNodes);
                }else{
...
function addDiyDom(treeId, treeNode) {
    ...
    var spanObj = $("#" + treeNode.tId + "_span").html(
        '<i class="i i-xing blockDian color_theme"></i>' + treeNode.docTypeName);
    spanObj
        .after('<button  class=" btn btn-link  download-btn color_theme" onclick="downloadZip(\''
            + treeId + '\',\'' + treeNode.tId + '\');">打包下载</button>');
    spanObj
        .after('<button  class=" btn btn-link pull-right formAdd-btn color_theme" data-toggle="collapse" data-target="#'
            + treeNode.tId
