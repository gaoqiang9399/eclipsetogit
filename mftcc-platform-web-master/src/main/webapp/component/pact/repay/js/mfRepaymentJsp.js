...
window.ajaxRepayment = function(obj, callback) {
	...
	if (flag) {
		...
		if(lsbqHuanBenFlag == 3 ){//利随本清还款 并且是 按照还款本金还款
			isWrong=ifCanReapaymentByLsbq();
		}else{
			// 验证是否能够还款 验证方法
			isWrong=ifCanReapayment();
		}
		...
		// 获取应收列表List的JSON字符串
		var loanReceivableBeanListRepaymentVar = getTianXieYingShouListInputTextNew();
		var dataParam = getMfRepaymentJsonStr();
...
function ifCanReapayment() {
	...
	// 获取本次冲抵金额
	var benCiCongDi = $("#benCiChongDi_input_text").val();
	// 还款方式
    ...
    benCiJieYu =  CalcUtil.formatMoney(benCiJieYu, '2');
    benCiJieYu = benCiJieYu.replace(/,/g, "");
	...
	if(lastTermBalanceType !="1"){//不允许
		var shiShouZongJiTotal =  CalcUtil.add(shiShouZongJi,youhuizongZongEr);
		if (shiShouZongJiTotal > totalPayVal * 1) {
			var timeOne = "实收总额,优惠合计";
			var timeTwo = "该笔数据应还总额";
			isWrong = true;
	...
	var jieyu = $("#benCiJieYu_input_text").val();// 本次结余
	jieyu =  CalcUtil.formatMoney(jieyu, '2');
    jieyu = jieyu.replace(/,/g, "");
	var chongdi = $("#benCiChongDi").text().replace(/,/g, "");
    ...
    //冲抵金额
	var chongdiAmt = $("#benCiChongDi_input_text").val();
	if ("2" == jieYuTuiKuanVal) {// 允许退款
		chongdiAmt = "0.00";
	}
	var shishouhejiTotal = CalcUtil.add(shishouheji,jieyu);
	var shishouzongeTotal = CalcUtil.add(shishouzonge,chongdiAmt);
	if (shishouhejiTotal != shishouzongeTotal) {// 勾选金额明细和结余合计与实收总额,优惠合计
		// 是否一致
		var money1 = "应收金额明细和结余合计";
 ...
 * @author WD
 */
function getTianXieYingShouListInputTextNew() {
	...
	$("#yingShou_List_table_001").find("tr[id^='tr_']").each(function(i, val) {
		...
		loanReceivableBean.benQiYingShouZongJi = benQiYingShouZongJi.replace(/,/g, "");
 ...
 * @returns
 */
function getMfRepaymentJsonStr() {
	var datas = [];
	var repaymentObj = {};
	// 借据号
...
function prepaymentJsp() {
	...
	$.ajax({
		...
		success : function(data) {
			if (data.flag == "success") {
				// 能够提前还款
				window.location.href = webPath+'/mfRepayment/mfPrepaymentJsp?fincId=' + fincId
			} else {// 不满足提前还款条件
				alert(data.msg, 0);
			}
		},
 ...
 * @author WD
 */
function ifCanReapaymentByLsbq() {
	var isWrong=false;
	// 获取利随本清还本金额
