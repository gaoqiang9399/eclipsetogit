$(function(){
	...
	if(operateflag=="1"||canRepay=="0"){//还款操作不能点击
        ...
        $("#registration").removeClass("btn-opt");
        $("#registration").addClass("btn-opt-dont");
    	...
    	$("#repay").attr("disabled",false);
  		...
  		$("#registration").removeClass("btn-opt-dont");
  		$("#registration").addClass("btn-opt");
        $("#buyerRepay").attr("disabled",false);
        $("#buyerRepay").removeClass("btn-opt-dont");
        $("#buyerRepay").addClass("btn-opt");
  		//退费按钮相关控制 refundFee 
		$("#refundFee").attr("disabled",false);
		$("#refundFee").removeClass("btn-opt-dont");
		$("#refundFee").addClass("btn-opt");
		//利随本清还款 按钮相关控制 LiXiRepay
	...
	if(fincSts=="7"){//业务完结,贷后检查不可操作
		...
		$("#compensatory").addClass("btn-opt-dont");
		$("#compensatory").removeClass("btn-opt");	
	...
	if(extenLiXiOperateFlag == "1"){// 不能操作
        $("#LiXiRepay").attr("disabled",true);
        $("#LiXiRepay").removeClass("btn-opt");
        $("#LiXiRepay").addClass("btn-opt-dont");
	}else{
...
//	getNextBusPoint();
	...
	MfBusRecourseApplyDetail.getRecourseType();
    getRepayTrialType();
    //非展期中或者展期审批中展期终止按钮均不可点击
    if (extenSts != "1" || extenApprove == "1") {
...
function setBlockTitle(showType,title,name){
	...
	//正常还款
	function repaymentRegistration(){
		top.flag=false;
		top.window.openBigForm(webPath+'/mfRepayment/repaymentRegistrationJsp?fincId='+fincId,'还款登记',repayCloseCallBack);
	}
	//买方付款
	function buyerRepayment(){
		top.flag=false;
		top.window.openBigForm(webPath+'/mfRepayment/repaymentJspForBuy?fincId='+fincId,'买方付款',repayCloseCallBack);
	}
	//尾款结付
	function tailPayment(){
		top.flag=false;
		top.window.openBigForm(webPath+'/mfRepayment/tailPayment?fincId='+fincId,'尾款结付',repayCloseCallBack);
	}
	//还款试算
	function repaymentTrial(){
		top.flag=false;
		top.window.openBigForm(webPath+'/mfRepayment/repaymentTrialJsp?fincId='+fincId,'还款信息',repayCloseCallBack);
	}


// 获取展期详情信息
function extensionDetail(obj, url) {

    top.window.openBigForm(webPath + url, '展期详情', function () {
			...
			success : function(data) {
				if (data.flag == "success") {
					if(preRepayApplyFlag=="1"){//需要提前还款申请
                        ...
                        top.window.openBigForm(webPath + '/mfPreRepayApply/getById?preRepayAppId=' + preRepayAppId + "&mfBusEditorRepayplanFlag=" + mfBusEditorRepayplanFlag, '提前还款信息', repayCloseCallBack);
					}else{
						// 能够提前还款
                        top.window.openBigForm(webPath + '/mfRepayment/mfPrepaymentJsp?fincId=' + fincId + "&mfBusEditorRepayplanFlag=" + mfBusEditorRepayplanFlag, '提前还款信息', repayCloseCallBack);
					}
				} else {// 不满足提前还款条件
					alert(data.msg, 0);
				}
...
// 					$("#LiXiRepay").removeClass("btn-opt");
// 					$("#LiXiRepay").addClass("btn-opt-dont");
// 				}else if(data.fincSts =="5") {//还款操作能点击
//                     $("#repay").attr("disabled", false);
//                     $("#repay").removeClass("btn-opt-dont");
//                     $("#repay").addClass("btn-opt");
//                     $("#prerepay").attr("disabled", false);
...
//                     $("#prerepay").addClass("btn-opt");
//                 }
//                 if(data.extenLiXiOperateFlag == "1"){
//                     //展期前收息后 按钮不能再次点击
//                     $("#LiXiRepay").attr("disabled",true);
//                     $("#LiXiRepay").removeClass("btn-opt");
//                     $("#LiXiRepay").addClass("btn-opt-dont");
// 				}
// 				if(top.preRepayApplyFlag=="1"){//更新提前还款申请表中的状态
// 					updatePreRepayApply(top.preRepayAppId);
// 				}
// 			},error:function(){
// 					alert(top.getMessage("ERROR_REQUEST_URL", getUrl));
// 				}
			...
			success:function(data){
				if(data.flag == "success"){
					...
					if(fiveclassSts == 0 || fiveclassSts == 1 || fiveclassSts == 4 || fiveclassSts == 5 || fiveclassSts == 3){//0系统初分 1不需要审批 5公司已认定
						...
						level = "L1";
					}else if(fiveclassSts == 6){
                        $("#fiveclass-i").text("阶段认定审批中");
                        $("#fiveclass").removeClass("btn-dodgerblue");
                        level = "L1";
                    }
					//暂不需要公司认定
...
function fileArchive(){
	...
	top.window.openBigForm(webPath+"/archiveInfoMain/input?pactId=" + pactId,"文件归档",function(){});
}
...
function queryEsignHistory(){
    top.window.openBigForm(webPath+"/mfEsignHistory/getListPage?appId=" + appId +"&cusNo="+cusNo,'电签记录查询', myclose);
}

// 提前还款页面跳转
function getRepayTrialType() {
    $.ajax({
        url:webPath+'/mfPreRepayApply/getRepayTrialType',
        data:{"fincId":fincId},
        type : "POST",
        dataType : "json",
        beforeSend : function() {
