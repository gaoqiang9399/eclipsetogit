...
<html>
	<head>
		...
		<script type="text/javascript" >
			var proName = '${proName}'||"";//项目标识
			$(function(){
			    myCustomScrollbar({
				obj : "#content", //页面内容绑定的id
				url:webPath+"/mfBusDeductFail/findByPageAjax", //列表数据查询的url
				tableId : "tabledeductfail0001", //列表数据查询的table编号
				tableType : "thirdTableTag", //table所需解析标签的种类
				pageSize:30 ,//加载默认行数(不填为系统默认行数)
				//,topHeight : 50 //顶部区域的高度，用于计算列表区域高度。
				callback:function(){
							if(proName && proName == 'KINGSOFT'){//金山项目展示
								$("#tab").children("tr").each(function(){
									var $this = $(this);
									$this.children("td").eq(-2).hide();
									$this.children("td").eq(-1).children("a").html("删除");
	...
	<script type="text/javascript" src="${webPath}/component/pact/js/MfBusDeductFail.js"></script>	
</html>
