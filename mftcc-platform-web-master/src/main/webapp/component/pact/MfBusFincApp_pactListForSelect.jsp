	...
	<head>
		...
		var cusNo = '${cusNo}';
			$(function(){
				...
				var table = "tablepactSelect";
				var url = webPath+"/mfBusFincApp/findPactByPageAjax";
			    myCustomScrollbar({
			...
			function choosePact(parm){
				parm=parm.split("?")[1];
				var parmArray=parm.split("&");
				var pactNo = parmArray[0].split("=")[1];
				$.ajax({
					url:webPath+"/mfBusFincApp/getPactInfoByPactNoAjax?pactNo="+pactNo,
					dataType:"json",
					type:"POST",
					success:function(data){
						if(data.flag == "success"){
							var obj = $.extend({},data.mfBusPact);
							parent.dialog.get('pactDialog').close(obj).remove();
						}else{
