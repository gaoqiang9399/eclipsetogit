...
$(document).ready(function() {
	resizeScreen();
	if(baseType=='4'){//渠道商,日后若资金机构也最为一种特殊客户类型，也可走此方法
		getSEData();
	}else if(baseType=='5'){
		getSEData();
	}else{
...
function getData() {
	...
	//cusNo为relation.jsp中声明的全局变量
	var url =webPath+"/mfCusRelation/ForView?cusNo="+cusNo;
    $.ajax({
        url:url,
        type: 'post',
        data:GetRequest(),
...
function getSEData() {
	//var url = basepath+getQueryString("jumpLink");
	//cusNo为relation.jsp中声明的全局变量
	var url =webPath+"/mfCusRelation/ForSEView?cusNo="+cusNo+"&baseType="+baseType;
    $.ajax({
        url:url,
        type: 'post',
        data:GetRequest(),
