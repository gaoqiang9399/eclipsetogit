...
$(function(){
	...
	$("body").show();
	$.ajax({
		url:webPath+"/tcReport/getWangxinReportAjax?cusNo="+cusNo,
		type:'post',
		dataType:'json',
		success:function(data){ 
			if(data.flag=="success"){
				if(data.shouxinMap){
					var shouxinMap=data.shouxinMap.data
					$("#basicInfo_amt").html(shouxinMap.pactAmt+"元");
					if(shouxinMap.pactTermType==1){
						$("#basicInfo_term").html(shouxinMap.pactTerm+"月");
					}else{
						$("#basicInfo_term").html(shouxinMap.pactTerm+"天");
					}
					$("#basicInfo_rate").html(shouxinMap.fincRate);
				}
				//客户基本信息
				if(data.mfCusPersBaseInfo){
					var mfCusPersBaseInfo=data.mfCusPersBaseInfo;
					var mfCusPersonJop=data.mfCusPersonJop;
					setMfCusPersBaseInfoHtml(mfCusPersBaseInfo,mfCusPersonJop);
				}
				if(data.familyInfoList.length==0){
					$("#familyInfo").html("暂无数据");
				}else{
					var tableHtmlPhoneBook=data.tableHtmlPhoneBook;
					$("#familyInfo").html(tableHtmlPhoneBook);
				}
				if(data.mfCusPersonAssetsInfoList==0){
					$("#mfCusPersonAssetsInfoList").html("暂无数据");
				}else{
					var mfCusPersonAssetsInfoList=data.mfCusPersonAssetsInfoList;
					setMfCusPersonAssetsInfoHtml(mfCusPersonAssetsInfoList);
				}
				//运营商
				if(data.operatorMap){
					var operatorMap=data.operatorMap;
					setOperatorMapHtml(operatorMap);
				}else{
					$("#operatorInfoContent").html("暂无数据");
				}
				//公积金
				if(data.gonjijinMap){
					var baseInfo=data.gonjijinMap.baseInfo;
					var billInfos=data.gonjijinMap.billInfos;
					setGongjijinHtml(baseInfo,billInfos);
				}else{
					$("#reservedFundInfoContent").append("暂无数据");
				}
				//芝麻分
				if(data.creditScore){
					var trHtml_z="";
					trHtml_z +='<tr>';
					trHtml_z += '<td class="DhbGetSauron_key">芝麻分</td><td align="center">'+data.creditScore+'</td>';
					trHtml_z += '<td class="DhbGetSauron_key"></td><td align="center"></td>';
					trHtml_z += "</tr><tr>";
					$("#sesamePointsInfoContent").append(trHtml_z);
				}else{
					$("#sesamePointsInfoContent").append("暂无数据");
				}
				///银联交易变量及评分
				
				if(data.unionpayMap){
					if(typeof(data.unionpayMap.data)!= "undefined"){
						var variableList=data.unionpayMap.data.VariableList;
						setunionpayTransactionHtml(variableList);
					}else{
						$("#unionpayTransactionContent").append("暂无数据");
					}
				}else{
					$("#unionpayTransactionContent").append("暂无数据");
				}
				//人失信黑名单
				if(data.personDishonestyMap.data){
					var personDishonesty=data.personDishonestyMap.data;
					$("#getPersonDishonestyContent").html(personDishonesty.resultMsg);
				}else{
					$("#getPersonDishonestyContent").html("非失信黑名单");
				}
				
				//个人黑名单
				if(data.personalBlacklistMap.data){
					var personalBlacklist=data.personalBlacklistMap.data;
					if(personalBlacklist.result==1){
						setHeimingdanHtml(personalBlacklist);
					}else{
						$("#getPersonalBlacklistContent").html(personalBlacklist.resultMsg);
					}
				}else{
					$("#getPersonalBlacklistContent").html("无黑名单信息");
				}
				//风云决
				setfenyunjueHtml(data.fengyunjueMap);
			}
		},error:function(){
...
function setMfCusPersBaseInfoHtml(mfCusPersBaseInfo,mfCusPersonJop){
	var trHtml="";
	trHtml +='<tr>';
	trHtml += '<td class="DhbGetSauron_key">客户名称</td><td align="center">'+mfCusPersBaseInfo.cusName+'</td>';
...
function setfenyunjueHtml(data){
	var trHtml_g="";
		...
		trHtml_g += "</tr><tr>";
		trHtml_g += '<td class="DhbGetSauron_key wx_report_left">申请前7天内的通话地点是否在（7天前到180天内的通话地点列表）之中验证结果</td><td align="center "><span class="wx_report_right">'+getFengyunjueItem(data.isLocOfLast7DayIn7To180Result)+'</span></td>';
