;
var MfCreditFlowConfig=function(window, $){
	//初始化产品基础配置信息
	...
	var getPrdctFlowConfigHtml = function(flowList){
		...
		+'<div class="main-content-desc"><span class="content-desc">流程设置主要是设置授信业务主流程、授信审批、授信合同清稿审批、授信合同审批流程的启用与否以及审批流程图</span></div>'
		...
		$.each(flowList,function(i,kindFlow){
			...
			var checkspan = '<span class="checkbox-span curChecked" data-id="'+kindFlow.kindFlowId+'" data-flowid="'+kindFlow.flowId+'" onclick="MfCreditFlowConfig.updateFlowUseFlag(this,\''+kindFlow.kindNo+'\',\'1\');"><i class="i i-gouxuan1"></i></span>';
			var alinkStr ='<a href="javascript:void(0);" onclick="MfCreditFlowConfig.openProcessDesigner(\''+kindFlow.flowId+'\')" class="padding_left_15 pointer">配置</a>';
			
			if(kindFlow.useFlag=="0"){
				checkspan='<span class="checkbox-span" data-id="'+kindFlow.kindFlowId+'" data-flowid="'+kindFlow.flowId+'" onclick="MfCreditFlowConfig.updateFlowUseFlag(this,\''+kindFlow.kindNo+'\',\'0\');"><i class="i i-gouxuan1"></i></span>';
			...
			success:function(data){
				if(data.flag=="success"){
					if(useflag=="0"){//禁用
						...
						$(obj).attr("onclick",'MfCreditFlowConfig.updateFlowUseFlag(this,\''+kindNo+'\',\''+useflag+'\');');
						//去掉a标签上的onclick事件
						...
						$(obj).removeAttr("onclick");
						$(obj).attr("onclick",'MfCreditFlowConfig.updateFlowUseFlag(this,\''+kindNo+'\',\''+useflag+'\');');
						var alinkStr ='<a href="javascript:void(0);" onclick="MfCreditFlowConfig.openProcessDesigner(\''+flowId+'\')" class="padding_left_15 pointer">配置</a>';
