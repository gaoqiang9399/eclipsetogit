;
var AgenciesShareProfit = function(window, $) {
	/**
	 * 在此处开始定义内部处理函数。
	 */
	var _init = function () {
		  url = webPath+"/mfBusFincApp/findByAgenciesAjax?cusNo=" + cusNo;//放款金额
		  tableId = "tableAgenciesShareProfitFincList";
		  if(calcBase == "1"){// 计算基数为客户数
			  url = webPath +"/mfCusCustomer/findByAgenciesAjax?agenciesUid=" + cusNo;
			  tableId = "tableAgenciesShareProfitCusList";
		  }
		  myCustomScrollbar({
		    	...
		    	callback:function(){
		    		$("th[class=table-float-th]").eq(0).html('<a href="javascript:void(0);" onclick="AgenciesShareProfit.checkAllBox()">全选</a>');
		    	}
		    });
		  $("th[class=table-float-th]").eq(0).html('<a href="javascript:void(0);" onclick="AgenciesShareProfit.checkAllBox()">全选</a>');
	};
	...
	var _ajaxSave = function(){
		...
		DIALOG.confirm("确定要对选中项进行分润操作么？", function(){
			$.ajax({
				url : webPath + "/mfCommissionChangeRecord/insertAgenciesAjax",
				data : {
