...
<script type="text/javascript">
	...
	function downloadFiles() {
		top.LoadingAnimate.start();
		var $checkboxes = $("#detail_div [name=archiveDetailNo]:checkbox:checked");
		if ($checkboxes.length > 0) {
			var archiveDetailNos = "";
			$checkboxes.each(function() {
				archiveDetailNos += $(this).val().split("=")[1] + "@";
			});
			archiveDetailNos = archiveDetailNos.substring(0, archiveDetailNos.length - 1);
			$.ajax({
				url:webPath+'/archiveInfoDetail/compressDownloadFileAjax?archiveMainNo=${archiveInfoMain.archiveMainNo}&archiveDetailNos=' + archiveDetailNos,
				type:'post',
				dataType:'json',
				success:function(data){
					if(data.success){
						window.location.href = webPath+"/archiveInfoMain/downloadFile?archiveMainNo=${archiveInfoMain.archiveMainNo}";
						setTimeout("top.LoadingAnimate.stop()", 500);
					} else {
						top.LoadingAnimate.stop();
						window.top.alert(data.msg, 0);
			...
			top.LoadingAnimate.stop();
			window.top.alert(top.getMessage("FIRST_SELECT_FIELD","要下载的文件"), 1);
		}
	}

	function previewFile(obj, url) {
		window.top.loadding();
		notie.alert(4, top.getMessage("WAIT_FILE_OPENING"), -1);
		$.ajax({
			url:url,
			type:'post',
			dataType:'json',
			success:function(data){
				if(data.success){
					var archiveDocAttribute = data.archiveInfoDetail.archiveDocAttribute;
					if (!archiveDocAttribute || archiveDocAttribute !== '') {
						if (archiveDocAttribute == '01') {//文档类型用pageOffice打开预览
...
// 									+ data.viewPath, data.archiveInfoDetail.docName, function(){}, "90", "90", "400", "300");
						...
						if(isWin){
							...
							window.top.alert(top.getMessage("ERROR_FILE_PREVIEW"), 0);
						}
					} else {
						window.top.alert(top.getMessage("ERROR_FILE_PREVIEW"), 0);
					}
					...
					<div class="row clearfix head-info">
						<div class="col-xs-3 column text-center head-img">
							<div style="position:relative;">
						 	...
						 	<%-- <c:if  test="%{archiveInfoMain.cusName.length()>16}">  
						    	<div class="btn btn-link" onclick="" title="${archiveInfoMain.cusName}"><s:property value="archiveInfoMain.cusName.substring(0,16)"/>... </div> 
						  	</c:if>  
						 	<s:else>  
								<div class="btn btn-link" onclick="">${archiveInfoMain.cusName}</div>
						  	</s:else>  --%>
							...
							<div class="row clearfix">
								<div class="col-xs-10 column">
									<div class="margin_bottom_5 clearfix">
										<div>
											<div class="bus head-title pull-left" style="cursor:hand" title="${archiveInfoMain.appName}">${archiveInfoMain.appName}</div>
										</div>
									</div>
									<div>
										<table>
											<tr>
												...
												<td>
													...
													<p><span id="term" class="content-span">${mfBusPact.term}</span><span class="unit-span margin_right_25"><c:if  test="${mfBusPact.termType=='1'}">个月</c:if><c:if  test='${mfBusPact.termType!="1"}'>天</c:if></span> </p>
												</td>
												<td>
													<p class="cont-title">合同利率</p>
													<p><span class="content-span">${mfBusPact.fincRate}</span><span class="unit-span">${rateUnit}</span></p>
												</td>
											</tr>
										</table>
									</div>
					...
					<div class="row clearfix btn-opt-group">
						<div class="col-xs-12 column">
							<div class="btn-group pull-right">
								<%--<button class="btn btn-opt" onclick="downloadFiles();" type="button">
									<i class="i i-xiazai"></i><span>下载</span>
								</button>
								<button id="sealButton" class="btn btn-opt" onclick="seal();" type="button">
									<i class="i i-fengdang"></i><span>封档</span>
								</button>--%>
							</div>
						</div>
					</div>
