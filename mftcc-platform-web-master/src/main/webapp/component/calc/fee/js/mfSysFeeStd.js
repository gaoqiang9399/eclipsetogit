...
$(function(){
		var bW = (parseInt($(document.body).outerWidth(true)/234))*234+5;
		$(".div_role_body").css("width",bW);
		var role = dataMap.mfSysFeeStdList;
	...
	(function($) {
		...
		//删除
		$.fn.role_btn_del1 = function(feeStdNo) {
			$(this).bind('click', function() {
				var $this = $(this).parents(".div_role_rotate");
				$.ajax({
					 type: "POST",
					 url: webPath+"/mfSysFeeStd/deleteAjax?feeStdNo="+feeStdNo,
		             dataType: "json",
		...
		$.fn.role_btn_update1 = function() {
			$(this).bind('click', function() {
				...
				var mydata = [];
				var postData = {};
				var feeStdNo = $this.find("#roleNo_back").val();
				var feeStdName = $this.find("#roleName_back").val();
				var useFlag = $this.find("input[name=useFlag]").is(':checked')?'1':'0';
				
				if(feeStdNo==""||feeStdName=="undefined" ||feeStdName=="" ||useFlag==""){
					alert("不能有空值！");
					return false;
				}
				
				postData.name = "feeStdNo";
				postData.value= feeStdNo;
				mydata.push(postData);
				
				postData = {};
				postData.name = "feeStdName";
				postData.value= feeStdName;
				mydata.push(postData);
				...
				$.ajax({
					 type: "POST",
		             url: webPath+"/mfSysFeeStd/updateAjax1",
		             data:{ajaxData:JSON.stringify(mydata)},
		             dataType: "json",
		             success: function(data){
		            	 $this.parent().find(".div_role_name").html(data.feeStdName);
		...
		$.fn.role_btn_add1 = function() {
			$(this).bind('click', function() {
				...
				$.ajax({
					type: "POST",
					url: webPath+"/mfSysFeeStd/insertAjax1",
					data:{ajaxData:JSON.stringify(mydata)},
					dataType: "json",
					success: function(data){
						if(data.sts==1){
		...
		$.fn.role_btn_view1 = function(roleNo) {
			$(this).bind("click",function(){
				...
				/*$.ajax({
					 type: "POST",
		             url: webPath+"/sysRole/getByRoleNo?roleNo="+roleNo,
		             dataType: "json",
		             success: function(data){
	...
	function new_role_obj(){
		...
		//添加按钮
		var $input_role_ctrl1_add = $("<div><label>费用标准号</label><input type='text'  name='roleNo_back' id='roleNo_back' value=''/></div>");
		var $input_role_ctrl2_add = $("<div><label>费用标准名</label><input type='text' name='roleName_back' id='roleName_back' value=''/></div>");
	...
	function add_role_obj(obj){
		...
		var $div_hover_pms = $('<a title="配置" alt="配置" href=webPath+"/mfSysFeeItem/getListPage?feeStdNo='+obj.feeStdNo+'" class="fa fa-gears"></a>');
		...
		var $div_role_no = $("<div class='div_role_no'>"+obj.feeStdNo+"</div>");
		var $div_role_name = $("<div class='div_role_name'>"+obj.feeStdName+"</div>");
		...
		//添加按钮
		var $input_role_ctrl1 = $("<div><label>费用标准号</label><input  type='text' readonly='readonly' name='roleNo_back' id='roleNo_back' value='"+obj.feeStdNo+"'/></div>");
		var $input_role_ctrl2 = $("<div><label>费用标准名</label><input type='text' name='roleName_back' id='roleName_back' value='"+obj.feeStdName+"'/></div>");
		...
		$div_hover_menu.role_btn_view1(obj.feeStdNo);//详情
		$div_hover_del.role_btn_del1(obj.feeStdNo);//绑定删除事件
...
//		$('.swraper.light .stoggler.off').css("background","#e7e7e7");
	...
	function searchRole(){
		...
		$.ajax({
			 type: "POST",
             url: webPath+"/mfSysFeeStd/search",
             data:{ajaxData:val},
             dataType: "json",
             success: function(data){
