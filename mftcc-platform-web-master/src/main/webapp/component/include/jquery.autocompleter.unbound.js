...
function prodAutoMenuUnbound(input,url,callback,fieldName,label,attach,limit){
...
//        }
        ...
        var html = '<div class="autocompleter '+opts.customClass.join(' ')+'" id="autocompleter-'+(guid+1)+'" style="width:'+($node.width()+5)+'px">';
            if (opts.hint) {
        ...
        // Bind node events
        data.$node.one("keyup.autocompleter", data, _onKeyup)
                  //.on("keydown.autocompleter", data, _onKeydownHelper)
                  .one("focus.autocompleter", data, _onFocus)
                  .one("blur.autocompleter", data, _onBlur);
//                  .one("mousedown.autocompleter", data, _onMousedown);
    ...
    $.fn.autocompleterunbound = function (method) {
        if (publics[method]) {
    ...
    $.autocompleterunbound = function (method) {
        if (method === "defaults") {
