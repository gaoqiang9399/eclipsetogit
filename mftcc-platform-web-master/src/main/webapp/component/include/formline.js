var allEle,headLen,subLen,formline;
	var allDD = [];
	var ddArr = [];
$(document).ready(function(){ 
		...
		var formline = $(".time-line-line").css("height",(headLen+subLen-1) * 37);
		allEle.each(function(i, list) {
			allDD.push($(this));
			var ddHtml;
			var name = $(this).attr('name');
				if (i == 0) {
					ddHtml = '<dd sub="false" class="time-line-dd" id="' + name +'">' + '<span class="time-line-dot line-dot-on"></span>' + '<a href="#' + name + '" class="time-line-a line-a-on">' + $(this).find("h3").html() + '</a>' + '</dd>';
					...
					ddHtml = '<dd sub="false" class="time-line-dd" id="' + name +'">' + '<span class="time-line-dot"></span>' + '<a href="#' + $(this).attr('name') + '" class="time-line-a">' +$(this).find("h3").html() + '</a>' + '</dd>';
					$(list).find(".formtith4").each(function(i){
						allDD.push($(this));
						var subname=name+"-"+i;
						$(this).attr("name",subname);
...
function scrollSelectorTop(name){
	var windowTop = $('[name="' + name + '"]').offset().top;
	$('body,html').animate({
		scrollTop: windowTop
	}, 'fast');
}
