...
var inputValue,selectFlag,cf = false;
function prodAutoMenu(input,url,callback,fieldName,label,isMust){
	...
	if(typeof(isMust)=="undefined"||isMust){
		selectFlag = true;
	}else if(!isMust){
		selectFlag = false;
	}
	...
	for(var i=0;i<arr.length;i++){
		template += "<span>"+attach+arr[i]+"</span>";
	}
//	$(input).focus(function(){
//		$(this).autocompleter("open");
//	});
	
	var fn = $(input).attr("name");
	...
	for(var i=0;i<arr.length;i++){
		if(i==0){
			...
			if($("input[name='"+value+"']").length>0){
				value = $("input[name='"+value+"']").eq(0).val();
				url += name+"="+value +"&";
			}else if($("select[name='"+value+"']").length>0){
				value = $("select[name='"+value+"']").eq(0).val();
				url += name+"="+value +"&";
			}else{
				url += name+"="+value +"&";
			}
...
//        }
    	...
    	//20170215 javelin update 下拉框top: 元素top + 元素高度；
    	var html = '<div class="autocompleter '+opts.customClass.join(' ')+'" id="autocompleter-'+(guid+1)+'" style="width:'+ ($node[0].offsetWidth) +'px;top:'+($node.offset().top+$node[0].offsetHeight)+'px;left:'+$node.offset().left+'px">';
            if (opts.hint) {
        ...
        /*$node.addClass("autocompleter-node")
             .after(html);*/
        //alert($(".autocompleter-list").width()+"|"+$node.width())
        if($("#autocompleter-"+(guid+1)).width()<$node.width()){
        	$("#autocompleter-"+(guid+1)).css("width",($node.width()+3));
        }
//        var $autocompleter = $node.next(".autocompleter").eq(0);
        var $autocompleter = $(".autocompleter").eq(0);
        ...
        data.$autocompleter.one("mousedown.autocompleter", ".autocompleter-item", data, _select)
                            .data("autocompleter", data);

        // Bind node events
        data.$node.unbind("keyup");
        data.$node.unbind("blur");
        data.$node.unbind("focus");
        data.$node.bind("keyup.autocompleter", data, _onKeyup)
                  //.on("keydown.autocompleter", data, _onKeydownHelper)
                  //.bind("focus.autocompleter", data, _onFocus)
                  .bind("blur.autocompleter", data, _onBlur);
                  //.one("mousedown.autocompleter", data, _onMousedown);
        
        data.$node.one("focus.autocompleter", data, _onFocus);
        
        /*if(cf){
        	data.$node.one("focus.autocompleter", data, _onFocus);
        }else{
        	data.$node.bind("focus.autocompleter", data, _onFocus);
        }*/
        data.$node.focus();
    }
    ...
    function _search(query, source, limit) {
        var response = [];
        //20170215 javelin update 防止query为非字符串； 
    ...
    function _searchresp(query, source, limit) {
        var response = [];
        query = query.toUpperCase();
        ...
        if (source.length) {
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                        switch (i) {
                            case 0:
                                if (typeof(source[item]) != "function" && source[item].label.toUpperCase().search(query) === 0) {
                                    response.push(source[item]);
                                    delete source[item];
                                }
                            break;
                            ...
                            case 1:
                                if (typeof(source[item]) != "function" && source[item].label.toUpperCase().search(query) !== -1) {
                                    response.push(source[item]);
                                    delete source[item];
                                }
                            break;
    ...
    function _launch(data) {
        ...
        //20170215 javelin update 支持传入数据源； 
        if (typeof data.source === 'object') {
            ...
            if(data.query.length === 0){
            	...
            	search = _search(data.query, _clone(data.source), data.limit);
            }
           	var searchresp = _searchresp(data.query, _clone(data.source), data.limit);
           	data.searchData = searchresp;
        	if (search.length) {
        		_response(search, data);
        	}
        } else {
        	var url = resUrl(data.source);
        	var ajaxData = $.extend({
        		limit: data.limit,
        		query: data.query
        	}, data.combine());
        	...
        	data.jqxhr = $.ajax({
        		...
        		data:{
        			limit: data.limit,
        			query: data.query,
        			ajaxData: data.fieldName
        		},
        		beforeSend: function (xhr) {
        			data.$autocompleter.addClass('autocompleter-ajax');
        			_clear(data);
        			if (data.cache) {
        				var stored = _getCache(url,data.query);
        				if (stored) {
        					xhr.abort();
        	...
        	.done(function (response) {
        		if (data.cache) {
        			_setCache(url, response.data,data.query);
        		}
    ...
    function _buildList(list, data) {
        ...
        //20170215 javelin update 去除每点击一次增加空白行； 
//        list.unshift({label:"&nbsp"});
        ...
        for (var item = 0, count = limit; item < count; item++) {
        ...
        if(list.length==1&&list[0].label=="&nbsp"){
        	menu = '<li data-label="" class="'+classes.join(' ')+' noResult">未找到结果</li>';
        }
    ...
    function _onBlur(e, internal) {
        ...
        //必选
        if(selectFlag){
            $("input[name="+data.$node.attr("name")+"]").val(inputValue);
        }
    }
    ...
    function _handleChange(data) {
    	...
    	if(data.response==null)
    		...
    		o = data.response[data.index];
        data.callback.call(data.$autocompleter, data.$node.val(), data, o);
     ...
     */
    function _setCache(url, data, query) {
     ...
     * @param url [string] "AJAX get query string"
     */
    function _getCache(url,query) {
        if (url&&query&&query!=""){
        	 var response = (cache[query] && cache[query].value) ? cache[query].value : false;
             return response;
        }
    }
