...
(function($, win) {
	var methods = {
					...
					$.ajax({
						...
						success: function(data) {
							...
							if($this.attr("disabled") && $this.attr("disabled") != "") {
								hideItem.attr('disabled', $this.attr("disabled"));
							}
		...
		showSelectValue: function(elem, options) { //赋初值
            ...
            if(options.multiple && options.selectAllItems){
                var nodeSelectAll = $(".pops-selectAll .option-input");
                var initSelectSize = popsparm.find(".pops-select-search .pops-label").length;
                var optionSize = options.items.length;
               	if(initSelectSize == optionSize && optionSize > 0){
                    nodeSelectAll.prop("checked", true);
                    $(".pops-selectAll .pops-item").addClass("pitch");
				}else{
                    nodeSelectAll.prop("checked", false);
                    $(".pops-selectAll .pops-item").removeClass("pitch");
				}
			}
		},
		...
		showValueDiv: function(elem, options) { //页面显示值区域
			...
			var icon = $('<span class="input-group-addon"><i class="i i-fangdajing pointer"></i></span>');
			icon.insertAfter(div);
		...
		buildSelectValue: function(elem, options) { //构建选择区域
            var html,items,selectHtml,search,addbtn;
			if(options.inline) {
				html = $(methods.getInlineSelectHtml());
				html.insertAfter(elem);
				items = $(methods.getInlineItemsHtml(options.items));
                ...
                if(options.multiple && options.selectAllItems) {
                    selectHtml = $(methods.getSelectAllItemsHtml());
                    html.find(".pops-select-head").append(selectHtml);
                    html.find(".pops-selectAll label").bind("click", function(e) {
                        if ($(e.target).is("input"))return;
                        methods.setChooseAllValue(options,elem);
                    });
                }
				if(options.searchOn) {
					search = $(methods.getSearchHtml());
				...
				html = $(methods.getOutlineSelectHtml(options));
				$("body").append(html);
				items = $(methods.getOutlineItemsHtml(options.items, options));
				html.find(".pops-select-body").append(items);
				elem.data("popsparm", html);
				items.find(".pops-item").bind("click", function() {
				...
				if(options.title) {
					...
					html.find(".pops-select-head").append('<div class="pops-select-title">'+$(elem[0]).attr('title')+"</div>");
				}
				if(options.addBtn && !$.isEmptyObject(options.addBtn)) {
					addbtn = $(methods.getTreeAddBtnHtml());
                ...
                if(options.multiple && options.selectAllItems) {
                    selectHtml = $(methods.getSelectAllItemsHtml());
                    html.find(".pops-select-head").append(selectHtml);
                    html.find(".pops-selectAll").unbind();
                    html.find(".pops-selectAll").bind("click", function(e) {
                        if ($(e.target).is("input"))return;
                        methods.setChooseAllValue(options,elem);
                    });
                }
		...
		showSelect: function(obj, elem) { //显示选择区域
			...
			if(elem.data("options").inline) {
				...
				if(typeof(options.itemsCount) == "number") {
					maxH = popsparm.find(".pops-item").not('[data-id=""]').outerHeight(true) * options.itemsCount;
					//多减了15，由于在系统中会遮住部门空白区域
					var tempH = win.innerHeight - popsparm.find(".pops-select-head").outerHeight() - popsparm.find(".pops-select-foot").outerHeight() - $(obj).offset().top - $(obj).outerHeight() -popsparm.find(".pops-item[data-id='']").outerHeight(true)- 20;
					if($(".formRowCenter")) {
						tempH -= $(".formRowCenter").outerHeight(true);
					}
					if(maxH > tempH) {
						popsparm.css("top", "");
						popsparm.css("bottom", $(obj).offset().top + "px");
					} else {
						popsparm.css("bottom", "");
						popsparm.css("top", ($(obj).outerHeight() + $(obj).offset().top) + "px");
					}
				} else {
        ...
        getSelectAllItemsHtml: function() { //获取全选html结构
			var html = [];
			html.push('<div class="pops-selectAll">');
            html.push('	<table class="pops-items"><tr><td class="pops-item">');
            html.push('<label title="全选" for="selectAll"><input id="selectAll" class="option-input option-radio checkbox" /><span class="name-span">全选</span><i class="i i-sanjiaoduihao"/></label>');
		...
		initEvent: function(elem) { //初始化事件
			$(win).bind("click", function(e) {
				if(!$(e.target).hasClass("pops-value") && !$(e.target).hasClass("pops-select") && $(e.target).parents(".pops-select").length == 0 && $(e.target).parents(".pops-value").length == 0 && !$(e.target).hasClass("pops-edit")) {
					methods.hideAll();
				}
			});
		},
		setSelectValue: function(options, elem, id) { //赋值
			...
			if(options.multiple) {
				...
				if(itemMap[id] && itemMap[id].name!=''){//选择内容后，删除不能为空的提示
		...
		getMultipleHtml: function(node, options) { //构建选中选择项label
			...
			html.push('<div class="pops-label-alt" title="' + itemMap[$(node).data("id")].name + '">');
		...
		setChooseValue: function(options, elem, node) { //设置pop选中值
			...
			if(options.multiple) {
				if(node.prop("checked")) {
					...
					label = $(methods.getMultipleHtml(node, options));
					label.appendTo(popsparm.find(".pops-select-search"));
					methods.setLabelEvent(label, options);
					popsparm.find(".pops-item[data-id='" + node.data("id") + "']").addClass("pitch");
				} else {
        ...
        setChooseAllValue: function(options,elem) {
			var nodeSelectAll = $(".pops-selectAll .option-input");
            var popsparm = elem.data("popsparm");
			if(nodeSelectAll.prop("checked")) {
                nodeSelectAll.prop("checked", false);
				$(".pops-selectAll .pops-item").removeClass("pitch");
                popsparm.find(".pops-select-body .pops-item").each(function(){
                    var node = $(this).find(".option-input");
                    methods.clearOneValue(options, node);
                    popsparm.find(".pops-item[data-id='" + node.data("id") + "']").removeClass("pitch");
                ...
                popsparm.find(".pops-select-body .pops-item").each(function(){
                    var node = $(this).find(".option-input");
					if(popsparm.find(".pops-select-search .pops-label[data-id='" + node.data("id") + "']").length > 0) return true;
					var label = $(methods.getMultipleHtml(node, options));
					label.appendTo(popsparm.find(".pops-select-search"));
					methods.setLabelEvent(label, options);
					popsparm.find(".pops-item[data-id='" + node.data("id") + "']").addClass("pitch");
		...
		updateItems: function(itemAry) { //更新选项
			this.each(function() {
				var $this = $(this);
				var options = $this.data("options");
				var popsparm = $this.data("popsparm");
				options.items = itemAry;
				methods.itemsToJson(options);
				$this.data("items", options.items);
				methods.buildSelectValue($this, options);
				methods.initEvent($this);
				...
				//设置默认选中值
				for(var k in itemAry ){
					if(itemAry[k].selected){
						var sltVal=itemAry[k].id;
						methods.setSelectValue(options, options.elem, sltVal);
					}
				}
			});
		...
		selectedById: function (val) {
			var items = this.data("items");
			if (items) {
				for ( var int = 0; int < items.length; int++) {
					if (items[int].id == val) {
						this.popupSelection("selectedItem", items[int]);
						return true;
					}
				}
			}
		...
		selectedItem: function(item) { //选中一个一选项
			this.each(function() {
				...
				if(options.inline) {
					if(options.multiple) {
						node = popsparm.find(".pops-item[data-id='" + item.id + "']");
						if(node.hasClass("on")) {
							node.removeClass("on");
					...
					if(options.multiple) {
						node = popsparm.find('.pops-item input[data-id=' + item.id + ']');
						node.prop("checked", true);
						if(popsparm.find(".pops-select-search .pops-label[data-id='" + node.data("id") + "']").length > 0) return false;
						label = $(methods.getMultipleHtml(node, options));
						label.appendTo(popsparm.find(".pops-select-search"));
		...
		popTreeInit: function(options, elem) { //ztree初始化
			...
			//poptree.find(".ztree").attr("id", "poptree-" + ztreeId);
		...
		initTreeValue: function(elem, options) { //赋初值
			...
			if(!options.multiple) {
				nodes = ztreeObj.getNodesByParam("id", arr[0], null);
				if(nodes.length > 0) {
				...
				var zNode = [];
				for(var i = 0; i < arr.length; i++) {
					nodes = ztreeObj.getNodesByParam("id", arr[i], null);
					if(nodes.length > 0) {
		...
		getTreeLabelHtml: function(node, options) {
			...
			html.push('<div class="pops-label-alt" title="' + node.name + '">');
			html.push(node.name);
			html.push('</div>');
			html.push('	<i class="pops-close"></i>');
			if(options.labelEdit && typeof(options.labelEdit) == "function") {
		...
		popTreeInitSearchEvent: function(options, elem) {
			...
			poptree.find(".pops-search-input").bind("keyup", function() {
				var val = poptree.find(".pops-search-input").val();
				methods.searchTreeNode(options, elem, val);
			});
		},
