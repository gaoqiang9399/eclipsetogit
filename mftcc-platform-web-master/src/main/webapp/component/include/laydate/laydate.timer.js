...
//主接口
win.laydateTimer = function(optionsArgs){
    var options = optionsArgs || {};
    try{
        as.event = win.event ? win.event : laydateTimer.caller.arguments[0];
    } catch(e){};
...
Dates.run = function(options){
    ...
    if (config.init) elem[as.elemv] = laydateTimer.now(null, options.format || config.format)
...
Dates.orien = function(obj, pos){
    ...
    //原来是rect.bottom + obj.offsetHeight/1.5,这样会导致页面大小不够,日历控件显示不全
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
        ...
        div.setAttribute('name', 'laydateTimer-v'+ laydateTimer.v);
        ...
        +'<div class="laydate_bottom">'
          ...
          +'</div>'
          +(config.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydateTimer-v'+ laydateTimer.v +'</a>' : '')
        +'</div>';
...
//重置定位
laydateTimer.reset = function(){
    (Dates.box && Dates.elem) && Dates.follow(Dates.box);
};

//返回指定日期
laydateTimer.now = function(timestamp, format){
...
//皮肤选择
laydateTimer.skin = function(lib){
