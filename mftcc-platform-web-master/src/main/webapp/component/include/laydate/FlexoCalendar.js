+(function($) {
	...
	var Calendar = function(element, options) {
		...
		this.type = null;
		this.calendar = '';
		var $target=$('<div class="flexoCalendar-box"></div>');
		$target.appendTo($(element).parent());
		$target.css("top",$(element).height()+"px");
		this.init($target[0], options);
	}

	Calendar.prototype.init = function(element, optionsArgs) {
		this.date = this.getDate();
		var options = this.opt = this.getOptions(optionsArgs, this.date);
	...
	Calendar.prototype.bulidFull = function(options, yearArgs, monthArgs) {
		var calendar = '';
		var month = monthArgs ? parseInt(monthArgs) : parseInt(options.setMonth);
		var year = yearArgs ? parseInt(yearArgs) : parseInt(options.setYear);
	...
	Calendar.prototype.bulidWeekly = function(options, yearArgs, monthArgs) {
		var calendar = '';
		...
		if (selectDate) {
			...
			if (typeof selectDate[2] == 'number') {
				selectDate[2] = eli.getMonthWeek(selectDate[0], selectDate[1], selectDate[2])
			}
		}
		var crt;
		for (var j = 0; j < 5; j++) {
			var weekNumber = this.getWeekNumber(startYear,startMonth,startDay);
			if (j != 0 && parseInt(startDay) >= parseInt(endDay)) {
				if (selectDate) {
					...
					if (($.isArray(selectDate[2]) && (year == selectDate[2][0][0] && month == selectDate[2][0][1] && selectDate[2][0][2] == crt)) || (year == selectDate[0] && month == selectDate[1] && selectDate[2] == crt)) {
						calendar += '<tr><td class="selected-week tweek week' + (j + 1) + '" data-time="'+ startYear + '-'+weekNumber+',' + startYear + '-' + this.dispose(startMonth) + '-' + this.dispose(startDay) + ',' + endYear + '-' + this.dispose(month) + '-' + this.dispose(endDay) + '" colspan="7"><div><span class="week">第' + weekNumber + '周</span><span class="process">' + this.dispose(startMonth) + '/' + this.dispose(startDay) + '~' + this.dispose(nextMonth) + '/' + this.dispose(endDay) + '</span></div></td></tr>';
					} else {
						calendar += '<tr><td class="tweek week' + (j + 1) + '" data-time="'+ startYear + '-'+weekNumber+',' + startYear + '-' + this.dispose(startMonth) + '-' + this.dispose(startDay) + ',' + endYear + '-' + this.dispose(nextMonth) + '-' + this.dispose(endDay) + '" colspan="7"><div><span class="week">第' + weekNumber  + '周</span><span class="process">' + this.dispose(startMonth) + '/' + this.dispose(startDay) + '~' + this.dispose(nextMonth) + '/' + this.dispose(endDay) + '</span></div></td></tr>';
					}
				} else {
					calendar += '<tr><td class="tweek week' + (j + 1) + '" data-time="'+ startYear + '-'+weekNumber+',' + startYear + '-' + this.dispose(startMonth) + '-' + this.dispose(startDay) + ',' + endYear + '-' + this.dispose(nextMonth) + '-' + this.dispose(endDay) + '" colspan="7"><div><span class="week">第' + weekNumber  + '周</span><span class="process">' + this.dispose(startMonth) + '/' + this.dispose(startDay) + '~' + this.dispose(nextMonth) + '/' + this.dispose(endDay) + '</span></div></td></tr>';
				}
				...
				if (selectDate) {
					...
					if (($.isArray(selectDate[2]) && ((year == selectDate[2][0][0] && month == selectDate[2][0][1] && selectDate[2][0][2] == crt) || (selectDate[2][1] && year == selectDate[2][1][0] && month == selectDate[2][1][1] && selectDate[2][1][2] == crt))) || (year == selectDate[0] && month == selectDate[1] && selectDate[2] == crt)) {
						calendar += '<tr><td class="current-week current-day selected-week tweek week' + (j + 1) + '" data-time="'+ startYear + '-'+weekNumber+',' + startYear + '-' + this.dispose(startMonth) + '-' + this.dispose(startDay) + ',' + endYear + '-' + this.dispose(month) + '-' + this.dispose(endDay) + '" colspan="7"><div><span class="week">第' + weekNumber  + '周</span><span class="process">' + this.dispose(startMonth) + '/' + this.dispose(startDay) + '~' + this.dispose(month) + '/' + this.dispose(endDay) + '</span></div></td></tr>';
					} else {
						calendar += '<tr><td class="current-week current-day tweek week' + (j + 1) + '" data-time="'+ startYear + '-'+weekNumber+',' + startYear + '-' + this.dispose(startMonth) + '-' + this.dispose(startDay) + ',' + endYear + '-' + this.dispose(month) + '-' + this.dispose(endDay) + '" colspan="7"><div><span class="week">第' + weekNumber  + '周</span><span class="process">' + this.dispose(startMonth) + '/' + this.dispose(startDay) + '~' + this.dispose(month) + '/' + this.dispose(endDay) + '</span></div></td></tr>';
					}
				} else {
					calendar += '<tr><td class="current-week current-day tweek week' + (j + 1) + '" data-time="'+ startYear + '-'+weekNumber+',' + startYear + '-' + this.dispose(startMonth) + '-' + this.dispose(startDay) + ',' + endYear + '-' + this.dispose(month) + '-' + this.dispose(endDay) + '" colspan="7"><div><span class="week">第' + weekNumber  + '周</span><span class="process">' + this.dispose(startMonth) + '/' + this.dispose(startDay) + '~' + this.dispose(month) + '/' + this.dispose(endDay) + '</span></div></td></tr>';
				}
	...
	Calendar.prototype.forWeek = function(yearArgs, month) {
		var endMonth = parseInt(month);
		var year = parseInt(yearArgs);
		var prevdate = this.getPrev(year, month, 'weekly');
		var monthLen = eli.getMonthLen(year, month);
		var prevMonth = prevdate.split('-')[1];
		var prevMonthLen = eli.getMonthLen(prevdate.split('-')[0], prevMonth);
	...
	Calendar.prototype.bulidMonthly = function(options, yearArgs, month) {
		var calendar = '';
	 ...
	 * @returns {Number}
	 */
	Calendar.prototype.getWeekNumber =function(y, m, d) {
...
+ (function($) {
	...
	eli.getMonthLen = function(yearArgs, monthArgs) {
		var year = parseInt(yearArgs);
		var month = parseInt(monthArgs);
	...
	eli.getMonthWeek = function(yearArgs, monthArgs, dayArgs) {
		var year = parseInt(yearArgs),
			month = parseInt(monthArgs),
			day = parseInt(dayArgs);
		var that = [ year, month, ],
...
$(function(){
	$(document).bind("click",function(e){
		 var data =$(e.target).data('ellie.calendar');
		 if(data||$(e.target).hasClass("flexoCalendar-box")||$(e.target).parents(".flexoCalendar-box").length>0){
			 
		 }else{
			 $(".flexoCalendar-box").hide();
		 }
	});
})
//简单调用方法
//onclick="laydateWeekly(this,'weekly',callback);"
//type=weekly 只显示第几周,type==all返回所有值,默认返回周一的日期
function laydateWeekly(obj,type,callback) {
	 $(".flexoCalendar-box").hide();
	 var data = $(obj).data('ellie.calendar');
	 if(data){
		 $(data.opt.target).show();
	 }else{
		$(obj).flexoCalendar({
			type : 'weekly',
			today : false,
			onselect : function(date) {
				if(type=="weekly"){
					$(obj).val(date.split(",")[0].replace("-", "-")+"");
					
				}else if(type=="all"){
					$(obj).val(date);
					
				}else{
					$(obj).val(date.split(",")[1]);
				}
				if(callback){
...
function laydateTest(date){
	console.log("回调方法返回值：",date);
}