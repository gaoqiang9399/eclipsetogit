...
//主接口
win.fPopUpCalendarDlg = function(options,syscallback){
    options = options || {};
    if(typeof(syscallback)=="function"){
        options.choose = syscallback;
    }
    try{
        as.event = win.event ? win.event : fPopUpCalendarDlg.caller.arguments[0];
    } catch(e){};
    if(win.event == null){
    	$(options.elem).removeAttr("onblur");
    }else{
    	$(as.event.target).removeAttr("onblur");
    }
    Dates.run(options);
    return fPopUpCalendarDlg;
};

fPopUpCalendarDlg.v = '1.1';
...
Dates.query = function(nodeArgs){
    ...
    var node = (Dates.trim(nodeArgs)).split(' '), elemId = doc[byid](node[0].substr(1)), arr;
    if(!elemId){
        return;
    } else if(!node[1]){
        return elemId;
...
Dates.run = function(options){
    ...
    if (config.init) elem[as.elemv] = fPopUpCalendarDlg.now(null, options.format || config.format);
    target.tagName = target.tagName == undefined?elem.tagName:target.tagName;
...
Dates.orien = function(obj, pos){
    ...
    //czk 20161215 原来是rect.bottom + obj.offsetHeight/1.5,这样会导致页面大小不够,日历控件显示不全
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
        ...
        div.setAttribute('name', 'fPopUpCalendarDlg-v'+ fPopUpCalendarDlg.v);
        ...
        +'<div class="laydate_bottom">'
          ...
          +'</div>'
          +(config.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">fPopUpCalendarDlg-v'+ fPopUpCalendarDlg.v +'</a>' : '')
        +'</div>';
...
Dates.close = function(){
	try {
		if (Dates.options.fireEvent){
			Dates.options.fireEvent('onchange');
		}else{
			Dates.options.onchange();
		}
	} catch (e) {
	}
    Dates.reshow();
    Dates.shde(Dates.query('#'+ as[0]), 1);
    if(typeof(func_uior_valTypeImm)=="function"){
    	func_uior_valTypeImm(Dates.elem);
    }
    $(Dates.elem).attr("onblur","func_uior_valTypeImm(this);");
    Dates.elem = null;
};
...
Dates.creation = function(ymd, hide){
    ...
    $(Dates.elem).keyup();
    Dates.elem.value=getDates;
    $(Dates.elem).parent().find("span[class='wrap-placeholder']").remove();
...
//重置定位
fPopUpCalendarDlg.reset = function(){
    (Dates.box && Dates.elem) && Dates.follow(Dates.box);
};

//返回指定日期
fPopUpCalendarDlg.now = function(timestamp, format){
...
//皮肤选择
fPopUpCalendarDlg.skin = function(lib){
    Dates.skinLink.href = Dates.getPath + as[4] + lib + as[5];
};

}(window);
function laydateTimer(obj, options){
	var opts = {elem: obj, istime: true, format: 'YYYY-MM-DD hh:mm:ss'};
	if (typeof options === "object") {
		opts = $.extend(opts, options);
		if (!!options.name) {
			opts.elem = $(obj).parents("form:eq(0)").find("input[name="+options.name+"]")[0];
		}
	}
