;
var VwBannerManageList = function(window, $) {
	/**
	 * 在此处开始定义内部处理函数。
	 */
	var _init = function () {
		myCustomScrollbar({
	    	...
	    	url:webPath+"/vwBannerManage/findByPageAjax",//列表数据查询的url
...
function finForm_input(url){//新增弹框
	top.addFlag = false;
	top.createShowDialog(url,"新增banner",'90','90',function(){
		if(top.addFlag){
			 updateTableData();//重新加载列表数据
   		}
	});
...
function finForm_detail(url){//详情弹框
	...
	top.createShowDialog(url,"banner详情",'90','90',function(){
		if(top.updateFlag){
			 updateTableData();//重新加载列表数据
   		}
	});
