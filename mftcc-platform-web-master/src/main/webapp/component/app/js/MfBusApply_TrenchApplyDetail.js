...
var MfBusApply_DynaDetail = function(window, $) {
	var $examinationReport = $('#examinationReport');
	var $secondReport = $('#secondReport');
	// 终止业务.不再使用。20180228沈浩兵
	...
	var _filePrint = function() {
		...
		top.window.openBigForm(webPath+"/mfTemplateBizConfig/getTemplateListPage?relNo=" + appId + "&modelNo=" + modelNo + "&generatePhase=" + encodeURI("01|02|03|04") + tempParm, '文件打印', myclose);
	};
	...
	// 预审批报告按钮初始化
	var _examinationReport_init = function() {
		if($examinationReport.length>0){//检查是否存在预审批报告按钮
			$.ajax({//查询预审批报告
				url : webPath+"/mfRiskManagement/getByIdAjax?appId=" + appId+"&returnId=1",
				data : {},
				type : 'post',
				dataType : 'json',
				async : true,
				success : function(data) {
					if (data.flag=="success") {//存在报告 
						if(data.state=="success"){//查询到报告是查询成功的
							
							$("#examinationReport").removeClass("btn-lightgray");// 去掉灰色样式
							$("#examinationReport").addClass("btn-forestgreen");// 添加绿色样式	
							$("#examinationReport").click(function(){ 
								top.window.openBigForm(webPath+"/mfRiskManagement/getexaminationApprovalContent?appId=" + appId +"&str=2"+"&query=query",'预审批查询', myclose);
								
							});
						}else{//查询到报告是查询失败的
							$("#examinationReport").removeClass("btn-lightgray");// 去掉灰色样式
							$("#examinationReport").addClass("btn-danger");// 添加红色样式	
							$("#examinationReport").click(function(){ 
								top.window.openBigForm(webPath+"/mfRiskManagement/getexaminationApprovalContent?appId=" + appId +"&str=2"+"&query=query",'预审批查询', myclose);
	...
	// 秒批报告按钮初始化
	var _secondReport_init = function() {
		if($secondReport.length>0){//检查是否存在秒批报告按钮
			$.ajax({//查询秒批报告
				url : webPath+"/mfRiskManagement/getByIdAjax?appId=" + appId+"&returnId=2",
				data : {},
				type : 'post',
				dataType : 'json',
				async : true,
				success : function(data) {
					if (data.flag=="success") {
						if(data.state=="success"){//查询到报告是查询成功的
							$("#secondReport").removeClass("btn-lightgray");// 去掉灰色样式
							$("#secondReport").addClass("btn-forestgreen");// 添加绿色样式	
							$("#secondReport").click(function(){  
								/* 在添加标签的同时给添加的标签绑定点击事件 */   
								top.window.openBigForm(webPath+"/mfRiskManagement/getsecondApprovalContent?appId=" + appId +"&str=1"+"&query=query",'秒批查询', myclose);
							});
						}else{//查询到报告是查询失败的
							$("#secondReport").removeClass("btn-lightgray");// 去掉灰色样式
							$("#secondReport").addClass("btn-danger");// 添加红色样式
							$("#secondReport").click(function(){  
								/* 在添加标签的同时给添加的标签绑定点击事件 */   
								top.window.openBigForm(webPath+"/mfRiskManagement/getsecondApprovalContent?appId=" + appId +"&str=1"+"&query=query",'秒批查询', myclose);
							});
...
//	//车辆实时位置查询
//	var _vehicleRealTimeLocation = function(){
//		top.window.openBigForm("MfThirdServiceRecordAction_thirdQueryTY.action?appId=" + appId +"&cusNo="+cusNo,'车辆实时位置查询', myclose);
//	};
	//经销商复核
	var _dealerReCheck = function(){
		var url = webPath+"/mfBusApply/dealerReCheckAjax?appId=" + appId;
		$.ajax({
			url : url,
			data : {},
			type : 'post',
	...
	return {
		...
		examinationReport_init:_examinationReport_init,
		secondReport_init:_secondReport_init,
//		vehicleRealTimeLocation:_vehicleRealTimeLocation,
		dealerReCheck:_dealerReCheck
	};
}(window, jQuery);
...
$(function(){
	...
	//MfBusApply_DynaDetail.examinationReport_init();// 尽调报告按钮初始化
	//MfBusApply_DynaDetail.secondReport_init();// 尽调报告按钮初始化
...
function getNextBusPoint(){
	...
	$.ajax({
		...
		success:function(data){
			...
			}else if(appSts=="2" && data.nodeName=="apply_approval"){//如果审批状态是通过，但是当前节点还是在业务审批阶段的话，需要手动触发一下业务流程的提交
			...
			//如果当前业务处理审批中，则终止业务按钮禁用不能操作
			if(data.nodeName=="apply_approval"){
				$("#MfBusApply_DynaDetail_disagree_button").attr("disabled", true);// 终止业务
				$("#MfBusApply_DynaDetail_disagree_button").attr("class", "btn btn-opt-dont");// 终止业务
			}
		}
...
function goToCollaDetailInfo(){
	if(top.collaFlag){
		if(top.vouType=="1"){
			...
			window.location.href=webPath+"/mfBusCollateralRel/getCollateralInfo?busEntrance=apply&cusNo="+cusNo+"&appId="+appId+"&relId="+appId+"&entrance="+entrance;
		}
	}
};
//选择押品类别
...
function getCusInfo(cusNoThis){
	top.LoadingAnimate.start();
	window.location.href=webPath+"/mfCusCustomer/getTrenchCusById?busEntrance="+busEntrance+"&cusNo="+cusNoThis+"&appId="+appId+"&operable="+operable;
}
...
function dblUpdateVal(keyArgs,data){
	if(keyArgs=="fincRate" || keyArgs=="overFloat"){//当申请利率或者逾期利率上浮发生变化时，逾期利率会联动计算
		...
		if(key=="fincRate"){
			fincRate = $(".changeval .inputText");
		}else if(key =="overFloat"){
			overFloat = $(".changeval .inputText");
		}
		$.ajax({
			url : webPath+"/mfBusApply/getOverOrCmpdRateAjax",
			data : {appId: appId,fincRate:fincRate,overFloat:overFloat},
			async : false,
			success : function(data) {
				if(data.flag=="success"){
...
function oneCallback(data) {
	...
	if(formAction == "MfCusPersonIncomeSurveyActionAjax_updateAjaxByOne.action") {//收入流水调查(铁甲网)
		$.ajax({
		...
		for(var i in data){
			...
			if(name=="coborrName" || name=="channelSource"){
				var dataParam = JSON.stringify($("#applyDetailForm").serializeArray());
				$.ajax({
			...
			if (name == "fincUse" && $(this).find("[name=fincUseSm]").length >= 1) {
				cleanAndGetFincUseSm(data[1].value,value, $_form);
				//getFincUseSm(value);
				/*$(this).find("[data-name=fincUseSm]").find(".filedShow").text("");
				$(this).find("[data-name=fincUseSm]").find(".filedReal select[name=fincUseSm]").text("");*/
			}
		}
	}
}
			...
			if (top.htmlStrFlag) {
				if (top.showType == "1") {
					...
					if (top.action == "MfCusBankAccManageAction") {
						$dynamicBlock.find(".content").html(top.htmlString+"<input id='updateByOneUrl' type='hidden' value='/mfCusBankAccManage/updateByOneAjax'></input>");
					}else{
...
//清空二级分类
function  cleanAndGetFincUseSm(appId, useId,$_form){
	$.ajax({
		url : webPath+"/mfBusApply/cleanAndGetFincUseSmAjax?appId="+appId+"&useId="+useId,
		type : "post",
		async : false,
		dataType : "json",
		success : function(data) {
			$("select[name=fincUseSm]", $_form).text("");
			$("[field-name=fincUseSm]", $_form).find(".fieldShow").text("");
			addTextToNone($("[field-name=fincUseSm]", $_form).find(".fieldShow"));
			//详细分类选择组件
			var items = data.items;
			for ( var index in items) {
				if(items[index].hasOwnProperty('useid')){
				$("select[name=fincUseSm]").append(
						"<option value="+ items[index].useid+">"
								+ items[index].usename
								+ "</option>");}
			}
		}
