		...
		<script type="text/javascript" >
			$(function(){
			   myCustomScrollbar({
			    	...
			    	url:webPath+"/mfPledgeClass/findByPageAjax",//列表数据查询的url
			...
			function getOpenFormInfo(obj,url){
				...
				var formtitle = "";
				var classNo = "";
				classNo = url.split("?")[1].split("=")[1];
				//暂不使用，母版确定后使用
				//formid_old = "ple"+classNo+formtype
				$.ajax({
					url:webPath+"/mfPledgeClass/getPleClasByIdAjax",
					data:{classNo:classNo,formtype:formtype},
					type:'post',
					dataType:'json',
					success:function(data){
						if(data.flag == "1"){
							...
							if(formid_new==""||formid_new==null){
								...
								openFormDesign(classNo,formtype,formtitle);
							}else{
			...
			function restoreForm(obj,url){
				var classNo = url.split("?")[1].split("=")[1];
			...
			function openFormDesign(classNo,formtype,formtitle){
				$.ajax({
					url:webPath+"/mfPledgeClass/doCopyForm",
					data:{formtype:formtype,formtitle:formtitle,classNo:classNo},
					type:'post',
					dataType:'json',
					success:function(data){
						if(data.flag == "success"){
