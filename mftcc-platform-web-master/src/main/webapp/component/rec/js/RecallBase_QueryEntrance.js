...
var RecallBaseQueryEntrance = function(window,$){
    var  applyBeginDate ="";
    var  applyEndDate = "";
    var _init = function(){
        //初始化滚动条
        ...
        $('input[name=fincShowId]').popupList({
            ...
            title: "选择借据",//标题
            searchplaceholder: "客户名称/合同号/借据号",//查询输入框的悬浮内容
    ...
    var _printApproveTable = function(obj,url){
        // debugger;
        var app_id =url.split("?")[1].split("&")[0].split("=")[1];
        var pact_id=url.split("?")[1].split("&")[1].split("=")[1];
        var finc_id=url.split("?")[1].split("&")[2].split("=")[1];
        var cus_no=url.split("?")[1].split("&")[3].split("=")[1];
        var cus_name=url.split("?")[1].split("&")[4].split("=")[1];
        var poCntJson = {
            filePath : "",
            fileName : "18110111582319595.doc",
            appId:app_id,//业务申请号
            pactId:pact_id,//合同编号
            fincId:finc_id,//借据号
            cusNo:cus_no,//客户号
            cusName:cus_name,//客户名称
            saveBtn : "0",
            fileType : 0
        };
