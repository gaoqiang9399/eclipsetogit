...
<script type="text/javascript">
	$(function(){
		...
		$.each(vouTypelist,function(i,vouType){
			...
			html = html + '<li class="risk_class_item item_false" data-voutype="'+vouType.optCode+'" data-voutypename="'+vouType.optName+'" title="'+vouType.optName+'" onclick="getNotSelectedItem(this);">'
		...
		$.each(optCode,function(i,optCodeThis){
			if(optCodeThis){
				var $obj = $("li[data-voutype="+optCodeThis+"]");
	...
	function confirm(){
		...
		$("li.risk_class_item.item_true").each(function(i,objThis){
			if(i == length-1){
				vouTypeNo = vouTypeNo + $(objThis).data("voutype");
				vouTypeDes = vouTypeDes + $(objThis).data("voutypename");
			}else{
				vouTypeNo = vouTypeNo + $(objThis).data("voutype")+"@";
				vouTypeDes = vouTypeDes + $(objThis).data("voutypename")+"@";
			}
		...
		parent.dialog.get("vouTypeMutiForPledgeDialog").close(vouType);
	};
</script>
</head>
