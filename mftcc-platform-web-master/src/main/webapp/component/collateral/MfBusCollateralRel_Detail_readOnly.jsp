		...
		<div class="row clearfix">	
			...
			<div class="col-md-8 column block-left">
				<div class="bg-white block-left-div">
							...
							<div class="row clearfix">
								...
								<div class="col-xs-10 column button-his">
									<div class="margin_bottom_5 clearfix">
										<div >
											<div class="bus-more head-title pull-left">${mfBusApply.appName}</div>
										</div>
									</div>
									<div class="margin_bottom_10">
										...
										<c:if test="${mfBusApply.busModel==5||mfBusApply.busModel==1}">
											<dhcc:pmsTag pmsId="pledge-prove">
												<button class="btn btn-view  btn-lightgray" type="button" id= "recePledge" onclick="MfBusCollateralRelDetailReadOnly.getRecePledgeInfo();">
										...
										<c:if test="${mfBusApply.busModel==13}">
											<dhcc:pmsTag pmsId="pledge-prove">
												<button class="btn btn-view  btn-lightgray" type="button" id= "recePledge" onclick="MfBusCollateralRelDetailReadOnly.getReceTranInfo();">
										...
										<c:if test="${mfBusApply.busModel==5||mfBusApply.busModel==13}">
											<dhcc:pmsTag pmsId="pledge-rebate-history">
												<button class="btn btn-view btn-lightgray" type="button" id="rebateHistory" onclick="MfBusCollateralRelDetailReadOnly.getRebateHistory();">
													<i class="i i-zherang1"></i><span>折让历史</span>
												</button>
											</dhcc:pmsTag>
											<dhcc:pmsTag pmsId="pledge-disputed-history">
												<button class="btn btn-view btn-lightgray" type="button" id="disputedHistory" onclick="MfBusCollateralRelDetailReadOnly.getDisputedHistory();">
										...
										<c:if test="${mfBusApply.busModel==1}">
											...
											<dhcc:pmsTag pmsId="pledge-modify-history">
												<button class="btn btn-view  btn-lightgray " type="button" id="modifyHistory" onclick="MfBusCollateralRelDetailReadOnly.getModifyHistory();">
										...
										<dhcc:pmsTag pmsId="pledge-repo-affirm-info">
											<button class="btn btn-view  btn-lightgray" type="button" id="repoAffirmInfo" onclick="MfBusCollateralRelDetailReadOnly.getRepoAffirmInfo();">
					...
					<c:if test="${mfBusCollateralRel.collateralType != 'account'}">
						<div class="row clearfix head-info" style="display:none">
							<div class="col-xs-3 column text-center head-img">
								<div>
									<button type="button" class="btn btn-font-pledge font-pledge-div padding_left_15">
							...
							<div class="col-xs-12 column">
								<div class="btn-group pull-right">
									<%-- <dhcc:pmsTag pmsId="add-pledge-info">
										<button class="btn btn-opt" id ="addCollateralInfo" onclick="MfBusCollateralRelDetailReadOnly.addCollateralInfo();" type="button">
												<i class="i i-jia1"></i><span>新增</span>
										</button>
									</dhcc:pmsTag>
									<dhcc:pmsTag pmsId="pledge-batch-inStock">
										<button id="batchInStock" class="btn btn-opt" onclick="MfBusCollateralRelDetailReadOnly.inStockCollateralBatch();" type="button">
									...
									<%-- <c:if test="${mfBusApply.busModel==4}">
										<dhcc:pmsTag pmsId="pledge-moveable-buy-back">
											<button id="moveableBuyBack" class="btn btn-opt" onclick="MfBusCollateralRelDetailReadOnly.moveableBuyBack();" type="button">
												<i class="i i-rebate"></i><span>回购申请</span>
											</button>
										</dhcc:pmsTag>
										<dhcc:pmsTag pmsId="pledge-moveable-buy-back-confirm">
											<button id="moveableBuyBackConfirm" class="btn btn-opt" style="display:none" onclick="MfBusCollateralRelDetailReadOnly.moveableBuyBackConfirm();" type="button">
									...
									<c:if test="${mfBusApply.busModel==13||mfBusApply.busModel==5}">
										<dhcc:pmsTag pmsId="pledge-rebate-apply">
											<button id="rebatePledge" class="btn btn-opt" onclick="MfBusCollateralRelDetailReadOnly.rebatePledge();" type="button">
												<i class="i i-zherang1"></i><span>折让</span>
											</button>
										</dhcc:pmsTag>
										<dhcc:pmsTag pmsId="pledge-rebate-affirm">
											<button id="rebateAffirm" class="btn btn-opt" style="display: none;" onclick="MfBusCollateralRelDetailReadOnly.rebateAffirm();" type="button">
												<i class="i i-zherang1"></i><span>折让确认</span>
											</button>
										</dhcc:pmsTag>
										<dhcc:pmsTag pmsId="pledge-disputed-deal-apply">
											<button id="disputedDealPledge" class="btn btn-opt" onclick="MfBusCollateralRelDetailReadOnly.disputedDealPledge();" type="button">
												<i class="i i-rebate"></i><span>争议处理</span>
											</button>
										</dhcc:pmsTag>
										<dhcc:pmsTag pmsId="pledge-relieve-disputed">
											<button id="relieveDisputed" class="btn btn-opt" style="display: none;" onclick="MfBusCollateralRelDetailReadOnly.relieveDisputed();" type="button">
									...
									<c:if test="${mfBusApply.busModel==13}">
										<dhcc:pmsTag pmsId="pledge-buy-back-apply">
											<button id ="buyBackPledge" class="btn btn-opt" onclick="MfBusCollateralRelDetailReadOnly.buyBackPledge();" type="button">
												<i class="i i-fanzhuanrang"></i><span>反转让</span>
											</button>
										</dhcc:pmsTag>
										<dhcc:pmsTag pmsId="pledge-rece-repo-affirm">
											<button id ="receRepoAffirm" class="btn btn-opt" style="display: none;" onclick="MfBusCollateralRelDetailReadOnly.receRepoAffirm();" type="button">
												<i class="i i-fanzhuanrang"></i><span>反转让确认</span>
											</button>
										</dhcc:pmsTag>
									</c:if>
									<dhcc:pmsTag pmsId="pledge-batch-out-stock">
										<button id="batchOutStock" class="btn btn-opt" onclick="MfBusCollateralRelDetailReadOnly.outStockCollateralBatch();" type="button">
			...
			<div class="col-md-4 column block-right">
				<div class="bg-white block-right-div">
					...
					<%-- <c:choose>
						<c:when test="${busEntrance == 'credit' || busEntrance== 'cus_credit' || busEntrance == 'cus_core_company'}">
					...
					<c:if test="${mfBusApply.busModel=='13'}">
						<jsp:include page="/component/collateral/receivables/MfBusCollateralRel_receivables_AbstractInfo.jsp"/>
					</c:if>
...
<script type="text/javascript" src="${webPath}/component/collateral/js/MfBusCollateralRel_Detail_readOnly.js"></script>
...
<script type="text/javascript">                
		...
		$(function(){
			MfBusCollateralRelDetailReadOnly.init();
		});
		</script>
		<script type="text/javascript">								
