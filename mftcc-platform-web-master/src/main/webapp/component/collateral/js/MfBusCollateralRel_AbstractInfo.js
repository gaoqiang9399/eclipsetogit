...
var  MfBusCollateralRel_AbstractInfo = function(window,$){
	...
	var _appId = '';
    var _extensionApplyId = '';
	var _init = function(){
		$.ajax({
			url:webPath+"/mfBusCollateralRel/getCollateralInfoAjax",
			data:{appId:MfBusCollateralRel_AbstractInfo.relId,cusNo:MfBusCollateralRel_AbstractInfo.cusNo},
			type:"POST",
			dataType:"json",
				...
				if (data.flag == "success") {
					...
					if(MfBusCollateralRel_AbstractInfo.relId!=""&&MfBusCollateralRel_AbstractInfo.relId!=null){
                        ...
                        if(MfBusCollateralRel_AbstractInfo.vouType=="1"||vouTypeShort=="1"){
                            ...
                            if(typeof(data.modifyQueryType)!="undefined"){
                            	var modifyQueryType = data.collateralRel.modifyQueryType;
                            	if ("query"==modifyQueryType){
                                    $("#addCollateralInfo").hide();
                                    $("#noPledge").show();
								}
							}
                        }
	...
	var _setCollateralInfo=function(collateralRel){
		...
		var htmlStr = '<div class="col-xs-3 col-md-3 column padding_top_10">'
				...
				+'<table><tbody><tr>'
				+'<td>'
				+'<p class="ptitle">抵押数量</p>'
				+'<p class="pvalue">'
				+'&nbsp;<span class="span-value" id="abstractInfo-envalueAmt">'+ collateralRel.num1+'</span>'
				+'</p>'
				+'</td>'
				+'<td>'
				+'<p class="ptitle">质押数量</p>'
				+'<p class="pvalue">'
				+'&nbsp;<span class="span-value" id="abstractInfo-receiptsAmount">'+ collateralRel.num2+'</span>'
				+'</p>'
				+'</td>'
				+'<td>'
				+'<p class="ptitle">保证数量</p>'
				+'<p class="pvalue">'
				+'&nbsp;<span class="span-value" id="abstractInfo-receiptsAmount">'+ collateralRel.num3+'</span>'
				+'</p>'
				+'</td>'
				+'</tr></tbody></table>'
				+'</div>'
	...
	var _getCollateralRelInfo=function(appId,busCollateralId){
		...
		window.location.href=webPath+"/mfBusCollateralRel/getCollateralInfo?busEntrance="+MfBusCollateralRel_AbstractInfo.busEntrance+"&operable="+MfBusCollateralRel_AbstractInfo.operable
            ...
            + "&entrance=" + MfBusCollateralRel_AbstractInfo.entrance + "&type=" + MfBusCollateralRel_AbstractInfo.type + "&fincId=" + MfBusCollateralRel_AbstractInfo.fincId + "&creditAppId=" + MfBusCollateralRel_AbstractInfo.creditAppId + "&extensionApplyId=" + MfBusCollateralRel_AbstractInfo.extensionApplyId;
	};
	...
	var _addCollateralInfo = function(){
		...
		if(MfBusCollateralRel_AbstractInfo.busEntrance == 'credit' 
            ...
            url = webPath + '/mfBusCollateralRel/insertInput?cusNo=' + MfBusCollateralRel_AbstractInfo.cusNo + "&appId=" + MfBusCollateralRel_AbstractInfo.relId + "&extensionApplyId=" + MfBusCollateralRel_AbstractInfo.extensionApplyId + "&entrFlag=businessNoTask";
		}
		top.openBigForm(url,"新增押品",function(){
			if(top.addCollateral){
				top.LoadingAnimate.start();
				window.location.href=webPath+"/mfBusCollateralRel/getCollateralInfo?cusNo="+MfBusCollateralRel_AbstractInfo.cusNo+"&appId="+MfBusCollateralRel_AbstractInfo.relId+"&relId="+MfBusCollateralRel_AbstractInfo.relId
	...
	return{
        ...
        appId: _appId,
        extensionApplyId: _extensionApplyId
	};
}(window,jQuery);