$.fn.collRcswitcher =  function(options){
	var reloadColl = function(){
		$spanArr.collRcswitcher(options);
	};
	...
	var setCollRcswitcher = function(){
		if(thName!=undefined){
							...
							$.ajax({
								...
								success:function(data){
									if(data.flag=="success"){
										var aHtml = "<a href='/mfCollateralClass/updateUserFlagAjax.action?classId="+
											data.classId+"' id='aaa'>"+data.useFlag+"</a>";
										$span.html(aHtml);
										reloadColl();
										alert(data.msg,1);
