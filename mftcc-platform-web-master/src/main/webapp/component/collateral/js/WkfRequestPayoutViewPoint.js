;
var WkfRequestPayoutViewPoint = function (window,$){
	var _init = function(){
	...
	var _doSubmitAjax = function(formObj){
		...
		if (flag) {
			...
			//必须对金额进行格式化
            // $("input[name=creditAmt_" + i).val().replace(/,/g, ""); 后台已做处理
			...
			commitProcess(webPath+"/mfRequestPayoutBill/submitUpdateAjax?appNo="+requestId+"&opinionType="+opinionType,formObj,'requestPayout');
		}
	};
	//返回详情页面
