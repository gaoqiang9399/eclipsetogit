...
<html>
	...
	<body class="body_bg">
		<div class="right_bg">
							...
							%>
								<tr >
									...
									<td width="5%" align="center">
										<%if( (task.getAssignee() == null || "".equals(task.getAssignee())) && "unsigned".equals(task.getSignState()) ) { %>
										<button onclick="javascript:lkconfirm('${webPath}/wkf/Task/take?taskId=<%=task.getId() %>')" class="ico_button4All" title="签收任务">签收任务</button>
										<%} else {%>
										<button onclick="javascript:openApproveUrl('<%=task.getId() %>','<%=task.getApproveUrl()%>')" class="ico_button4All" title="审批">审批</button>
										<%} %>
										<button onclick="javascript:approveIdea('<%=task.getExecutionId()%>')" class="ico_button4All" title="查看已审批意见">查看意见</button>
			...
			<pageC:page urlPath="${webPath}/task/myProxyTask" scope="request" page="ipage" />
							<!--list end-->
						</div>
						<!--table end -->
					</form>
...
<script language="javascript" >
	function openApproveUrl(taskId, approveUrl) {
		var url = '<%=webPath %>/wkf/Task/<%=WF.PARAM_TASK_OPEN_APPROVEURL%>?taskId=' + taskId + '&approveURL=' + encodeURIComponent(approveUrl);
		buttonforward(url);
	}
</script>
