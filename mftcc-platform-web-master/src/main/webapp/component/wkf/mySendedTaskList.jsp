	...
	<body class="body_bg">
		<div class="right_bg">
			<div class="right_w">
				<!--con -->
				<div class="from_bg">
					<form id="${webPath}/task/mySendedTask" name="cms_form" action="<%=webPath %>/wkf/Task/mySendedTask" method="post">
						<!--table -->
						<div class="table_w">
							<!--list -->
							...
							%>
								<tr >
									...
									<td width="5%" align="center">
										<%if(!NodeType.SIGNTASK.equals(task.getActivityType()) ) {%>
										<button onclick="javascript:lkconfirm('<%=webPath %>/wkf/Task/<%=WF.PARAM_TASK_RECALL %>?taskId=<%=task.getId() %>')" class="ico_button4All" title="召回">召回</button>
										<%} %>
			...
			<pageC:page urlPath="${webPath}/task/mySendedTask" scope="request" page="ipage" />
							<!--list end-->
						</div>
						<!--table end -->
					</form>
