;
var JKInstitutions_List = function(window, $) {
	var _init = function() {
		   myCustomScrollbar({
				obj : "#content", //页面内容绑定的id
				url : webPath+"/jKInstitutions/findByPageAjax", //列表数据查询的url
	...
	var _applyInsert = function() {
		top.openBigForm(webPath+"/jKInstitutions/input","新增机构管理", function(){
 			updateTableData();
 		});	
	};
	//跳转至详情
	var _dimissionDetail = function(obj,url) {
		...
		top.openBigForm(url+"&entryFlag=1","机构管理详情", function(){
 			updateTableData();
 		});	
	};
