...
;(function ($, window) {
     ...
     */
    function _search(query, source, limit,sreachFields,tempsearch) {
    	if(typeof(tempsearch)!="undefined"){
    		source = tempsearch;
    	}
        var response = [];
        query = query.toUpperCase();
        var j,sourceData,index;
        if (source.length) {
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                    if (limit&&response.length < limit) {
                        switch (i) {
                            case 0:
                            	for(j in sreachFields){
                            		sourceData = _expr(source[item][sreachFields[j]]);
                            		try {
                            			index = sourceData.toUpperCase().search(query);
									} catch (e) {
										index = -1;
									}
                            		 if (index === 0) {
        ...
        if (!data.empty && data.query.length === 0) {
            ...
            if (typeof data.source === 'object') {
                ...
                for(var q=0;q<queryArr.length;q++){
                	tempsearch = _search(queryArr[q], _clone(data.source), null,data.sreachField,tempsearch);
                }
                var limitNum;
                if(tempsearch<data.limit){
                	limitNum = tempsearch;
                }else{
                	limitNum = data.limit;
                }
                for(var i =0;i<limitNum;i++){
                	search.push(tempsearch[i]);
                }
               	data.searchData = tempsearch;
               	searchDatas = tempsearch;
                if (search.length) {
    ...
    function _buildList(list, data) {
        ...
        for (var item = 0, count = list.length; item < count; item++) {
        	if(list[item]===undefined){ continue;}
...
//                template = _expr(template);
        ...
        if (list.length && data.hint) {
            var hint = ( list[0][data.customLabel].substr(0, data.query.length).toUpperCase() === data.query.toUpperCase() ) ? list[0][data.customLabel] : false;
            if (hint && (data.query !== list[0][data.customLabel])) {
    ...
    function _expr(str){
    	var t=/(\(|（)[^（\(\)）]*?(\)|）)/g;//比配成对的括号
    	while(str.match(t))str=str.replace(t,'');
