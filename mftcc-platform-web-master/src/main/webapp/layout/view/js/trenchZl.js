	 	...
	 	$.extend(this, {
	 		...
	 		addMenuEvent:function(selector,event){
	 			$(selector).bind(event,function(){
	 				...
	 				/*//点击进件菜单,刷新进件菜单上业务数量
	 				if(GetQueryString("entranceNo",a.attr("href"))=="2"){
	 					showBusinessCount.applyCount();
	 				}
	 				//点击签约菜单,刷新签约菜单上业务数量
	 				if(GetQueryString("entranceNo",a.attr("href"))=="3"){
	 					showBusinessCount.pactCount();
	 				}*/
	 				//如果点击的是设置，当前菜单样式不变
...
//	 							window.open("/factor/layout/view/setView.jsp");
		 						...
		 						$.get(webPath+"/cwZtBooks/chooseFinBooksAjax", function(data){
		 							if(data.flag=='success'){
		 								...
		 								if(data.setSession){
		 									window.open("cwView.action", "cwView");
		 								}else{
		 									top.selectFlag = false;
		 									top.createShowDialog("CwZtBooksAction_getBooksSelectList.action", "选择帐套",'50','50',function(){
		 										if(top.selectFlag){
		 											window.open("cwView.action", "cwView");
		 										}
		 									});
...
//				pageNum = flag;
	 		...
	 		initMenu:function(selector){
	 			...
	 			var isC = false;
	 			var bussCountStr="";
	 			if(viewMenuData!=null){
	 				for(var i = 0,j =viewMenuData.length;i<j;i++){
	 					var entranceNo=GetQueryString("entranceNo",viewMenuData[i].url);
	 					bussCountStr="";
	 					if(entranceNo=="2"){
	 						bussCountStr="<span class='badge buss-count' id='apply_count' style='display:none'></span>";
	 					}else if(entranceNo=="3"){
	 						bussCountStr="<span class='badge buss-count' id='pact_count' style='display:none'></span>";
	 					}
	 					$(selector).append("<li><i class='i i-"+viewMenuData[i].css+"'></i>"+
	 							"<a href='"+path+"/"+viewMenuData[i].url+"'pageType='"+viewMenuData[i].css+"' onclick='return false'>"+bussCountStr+
	 							"<span>"+viewMenuData[i].name+"</span></a></li>");
	 			...
	 			if(viewFlag=="view"){
	 				if(homePage=="1"){
	 					rzzl.skipPage(webPath+"/mfTrenchUser/getTrenchCenterInfo?opNo="+opNo);
