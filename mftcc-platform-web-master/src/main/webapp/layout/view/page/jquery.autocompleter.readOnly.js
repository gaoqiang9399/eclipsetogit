function prodAutoMenu(input,url,callback){
	...
	$(input).autocompleter({
        ...
        source: url,
        //template: '{{ label }} <span>({{ '+attach+' }})</span>',
        selectfirst: true,
        ...
        callback: function (value, data, selected) {
        	if(selected){//单选
        		if (data.$node.hasClass("Required")) {
    				data.$node.removeClass("Required");
    			}
    			if(data.$node.parent().find(".Required-font").length>0){
    				data.$node.parent().find(".Required-font").remove();
    			}
...
var searchDatas,timeout,isMouseDown = false;
;(function ($, window) {
    "use strict";
        ...
        if (typeof data.source === 'object') {
            ...
            data.jqxhr = $.ajax({
                ...
                beforeSend: function (xhr) {
                    ...
                    _clear(data);
                    if (data.cache) {
                        var stored = _getCache(this.url,data.query);
                        if (typeof(stored)!="undefined"&&stored.length>0) {
                            xhr.abort();
            ...
            .done(function (response) {
                if (data.cache) {
                    _setCache(this.url, response,data.query);
                }
    ...
    function _onBlur(e, internal) {
        ...
        var si = setInterval(function(){
        	...
        	if(json!=null&&json[data.$node.val()]){
        		clearInterval(si);
        		for(var i in data.response){
        			if(data.response[i].label==data.query){
	            		flag = true;
	            	}
        		}
        		if(!flag){
        			//data.$node.val("");
        			var msg = "数据不存在!";
        			var $label = $('<label class="Required-font"><i class="fa fa-exclamation-circle"></i>'+msg+'</label>');
        			if(data.$node.parent().find(".Required-font").length==0){
        				$label.appendTo(data.$node.parent());
        			}
        			data.$node.addClass("Required");
        		}else{
        			if (data.$node.hasClass("Required")) {
        				data.$node.removeClass("Required");
        			}
