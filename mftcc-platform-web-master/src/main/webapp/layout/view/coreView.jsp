...
<html>
  <head>
        ...
        function sysQuit(){
            $.ajax({
                url :  webPath+"/sysCoreLogin/logout",
                type : 'post',
                dataType : 'json',
                success : function(data) {
                    if (data.flag == "success") {
                        window.top.alert("退出成功", 1);
                        window.top.location.href = webPath+"/sysCoreLogin/coreLogin";
                    } else {
                        alert(data.msg, 0);
                    }
                },
...
<script type="text/javascript">
	 ...
	 var coreCompanyUid='${coreCompanyUid}';
	 var coreCompanyId='${coreCompanyId}';
