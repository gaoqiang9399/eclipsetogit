(function($) {
	...
	*********************************************/
	...
	function Qutton(jQueryDOMElement) {
		...
		this.$dialog = this.$container.children(".qutton_dialog");
		// Cache the close button if it exists
		this.$closeButton = this.$container.find('.qutton_close');
		...
		this.quttonConfig = {
			...
			backgroundColor : this.$container.css("background-color"),
	...
	Qutton.prototype.setQuttonConfig = function (quttonConfig){
	...
	Qutton.prototype.init = function(quttonConfig) {
		...
		this.events.dblclick.call(this);
//		this.events.click_document.call(this);
	...
	Qutton.prototype.closeDialog = function() {
		var json = golFlow.$nodeData[this.$container.parent()[0].id];
		if(typeof(json)!="undefined"){
			this.quttonConfig.width = json.width;
			this.quttonConfig.height = json.height;
		}
		if(this.$container.hasClass("qutton_open")){
			this.$container.removeClass("qutton_open");
			this.setIcon();
			this.animateOut(this.$container.attr("nodeType"));
		}
	};

	Qutton.prototype.openDialog = function() {
		if(!this.$container.hasClass("qutton_open")){
			this.$container.addClass("qutton_open");
			this.removeIcon();
			this.animateIn();
		}
	};
	...
	Qutton.prototype.animateIn = function() {
		...
		var translate = {
			...
			Y : -1 * (this.dialogConfig.height/2 - this.quttonConfig.width/2)
		};

		var w = this.$container.parent().children("table").width()+50;
		if(this.$container.attr("id")!="processProp"){
			translate.X=w;
			translate.Y=0;
		}
		var inSequence  = [
	...
	Qutton.prototype.animateOut = function(type) {
		var that = this;
		var outSequence = [
