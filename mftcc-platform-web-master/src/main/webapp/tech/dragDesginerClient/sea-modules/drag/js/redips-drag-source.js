...
REDIPS.drag = (function () {
	...
	// (object) event handlers
		event = {changed : function () {},
			...
			switched : function () {},
			undeleted : function () {},
			setUsableTd:function(){},
			handlerOnMouseDown:function(){},
			dragOnMouseOver:function(){}};
	...
	init = function (dc) {
		...
		tds =  document.getElementById("table2").getElementsByTagName('td');
		// disable onmousemove/ontouchmove event for images to prevent default action of onmousemove event (needed for IE to enable dragging on image)
		for (i = 0; i < tds.length; i++) {
				REDIPS.event.add(tds[i], 'mousemove', function () {
					REDIPS.drag.event.dragOnMouseOver(this);
				});
				REDIPS.event.add(tds[i], 'touchmove', function () {
					REDIPS.drag.event.dragOnMouseOver(this);
				});
		}
	};
	...
	dragOnMouseOver =function(){
		REDIPS.drag.event.dragOnMouseOver();
	}
	/**
	...
	handlerOnMouseDown = function (e) {
		...
		REDIPS.drag.event.handlerOnMouseDown();
		return false;
	};
	...
	registerEvents = function (div, flag) {
		...
		if (flag === false) {
			...
			//div.onmousemove = dragOnMouseOver;
			//div.onmousemove = function(){REDIPS.drag.event.dragOnMouseOver(div)};
		}
	};
	...
	setPosition = function () {
		...
		if (table < tables.length && table !== null && row !== null && cell !== null) {
			...
			//REDIPS.drag.event.setBgColor(row,cell,tables[table].rows[row].cells[cell].style.backgroundColor);
	...
	setTdStyle = function (ti, ri, ci, t) {
		...
		if (mode === 'cell' && threshold.flag) {
			...
			REDIPS.drag.event.setUsableTd(ti,row,cell,s.backgroundColor);
