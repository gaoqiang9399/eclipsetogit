...
$.extend($.ui, { monthPicker: { version: "1.10.1" } });
...
function MonthPicker() {
	...
	this._inlineClass = "ui-monthpicker-inline"; // The name of the inline marker class
	this._appendClass = "ui-monthpicker-append"; // The name of the append marker class
	...
	this._disableClass = "ui-monthpicker-disabled"; // The name of the disabled covering marker class
	this._unselectableClass = "ui-monthpicker-unselectable"; // The name of the unselectable cell marker class
	this._currentClass = "ui-monthpicker-current-day"; // The name of the current day marker class
	this._dayOverClass = "ui-monthpicker-days-cell-over"; // The name of the day hover marker class
	...
	this.dpDiv = bindHover($("<div id='" + this._mainDivId + "' class='ui-monthpicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all'></div>"));
}
...
$.extend(MonthPicker.prototype, {
	...
	_newInst: function(target, inline) {
		...
		return {id: id, input: target, // associated target
			...
			bindHover($("<div class='" + this._inlineClass + " ui-monthpicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all'></div>")))};
	},
	...
	_enableMonthPicker: function(target) {
		...
		if (nodeName === "input") {
			...
			inline.find("select.ui-monthpicker-year").
				prop("disabled", false);
		}
	...
	_doKeyDown: function(event) {
		var onSelect, dateStr, sel,
			...
			handled = true,
			isRTL = inst.dpDiv.is(".ui-monthpicker-rtl");
	...
	_showMonthPicker: function(input) {
		...
		if (!$.monthPicker._pos) { // position below input
			...
			if(navigator.appName == "Microsoft Internet Explorer"){
				...
				$.monthPicker._pos[1] += input.offsetHeight+scrollY; // add the height
			} else {
	...
	_updateMonthPicker: function(inst) {
		...
		if (cols > 1) {
			inst.dpDiv.addClass("ui-monthpicker-multi-" + cols).css("width", (width * cols) + "em");
		}
		inst.dpDiv[(numMonths[0] !== 1 || numMonths[1] !== 1 ? "add" : "remove") +
			"Class"]("ui-monthpicker-multi");
		inst.dpDiv[(this._get(inst, "isRTL") ? "add" : "remove") +
			"Class"]("ui-monthpicker-rtl");
	...
	_selectDay: function(id, month, year, td) {
		...
		inst = this._getInst(target[0]);
		inst.selectedDay = inst.currentDay = "10";
	...
	_attachHandlers: function(inst) {
		...
		inst.dpDiv.find("[data-handler]").map(function () {
			var handler = {
				prev: function () {
					window["DP_jQuery_" + dpuuid].monthPicker._adjustDate(id, -stepMonths, "Y");
				},
				next: function () {
					window["DP_jQuery_" + dpuuid].monthPicker._adjustDate(id, +stepMonths, "Y");
				},
				hide: function () {
					window["DP_jQuery_" + dpuuid].monthPicker._hideMonthPicker();
				},
				today: function () {
					inst.input.val(getNowMonth());
					window["DP_jQuery_" + dpuuid].monthPicker._hideMonthPicker();
				},
				clean: function () {
					inst.input.val("");
				...
				selectDay: function () {
					window["DP_jQuery_" + dpuuid].monthPicker._selectDay(id, +this.getAttribute("data-month"), +this.getAttribute("data-year"), this);
					return false;
				},
				selectMonth: function () {
					window["DP_jQuery_" + dpuuid].monthPicker._selectMonthYear(id, this, "M");
					return false;
				},
				selectYear: function () {
					window["DP_jQuery_" + dpuuid].monthPicker._selectMonthYear(id, this, "Y");
					return false;
				}
			};
	...
	_generateHTML: function(inst) {
		...
		prev = (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ?
			"<a class='ui-monthpicker-prev ui-corner-all' data-handler='prev' data-event='click'" +
			" title='" + prevText + "'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "e" : "w") + "'>" + prevText + "</span></a>" :
			(hideIfNoPrevNext ? "" : "<a class='ui-monthpicker-prev ui-corner-all ui-state-disabled' title='"+ prevText +"'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "e" : "w") + "'>" + prevText + "</span></a>"));
		...
		next = (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
			"<a class='ui-monthpicker-next ui-corner-all' data-handler='next' data-event='click'" +
			" title='" + nextText + "'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "w" : "e") + "'>" + nextText + "</span></a>" :
			(hideIfNoPrevNext ? "" : "<a class='ui-monthpicker-next ui-corner-all ui-state-disabled' title='"+ nextText + "'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "w" : "e") + "'>" + nextText + "</span></a>"));
		...
		controls = (!inst.inline ? "<button type='button' class='ui-monthpicker-close ui-state-default ui-priority-secondary ui-corner-all' data-handler='hide' data-event='click'>" +
			this._get(inst, "closeText") + "</button>" : "");

		buttonPanel = (showButtonPanel) ? "<div class='ui-monthpicker-buttonpane ui-widget-content'>" + (isRTL ? controls : "") +
			(this._isInRange(inst, gotoDate) ? 
			"<button type='button' class='ui-monthpicker-clean ui-state-default ui-priority-secondary ui-corner-all' data-handler='clean' data-event='click'" +
			">" + this._get(inst, "clearText") + "</button>" +
			"<button type='button' class='ui-monthpicker-current ui-state-default ui-priority-secondary ui-corner-all' data-handler='today' data-event='click'" +
			">" + getNowMonth() + "</button>" : "")+
			(isRTL ? "" : controls) + "</div>" : "";
		...
		for (row = 0; row < numMonths[0]; row++) {
			...
			for (col = 0; col < numMonths[1]; col++) {
				...
				calender = "";
				if (isMultiMonth) {
					calender += "<div class='ui-monthpicker-group";
					if (numMonths[1] > 1) {
						switch (col) {
							case 0: calender += " ui-monthpicker-group-first";
								cornerClass = " ui-corner-" + (isRTL ? "right" : "left"); break;
							case numMonths[1]-1: calender += " ui-monthpicker-group-last";
								cornerClass = " ui-corner-" + (isRTL ? "left" : "right"); break;
							default: calender += " ui-monthpicker-group-middle"; cornerClass = ""; break;
						}
					}
					calender += "'>";
				}
				calender += "<div class='ui-monthpicker-header ui-widget-header ui-helper-clearfix" + cornerClass + "'>" +
					...
					"</div><table class='ui-monthpicker-calendar'>";
				calender += "<tbody>";
				...
				for (var cMon = 0; cMon < monthNames.length; cMon++) { // create date picker rows
					if(cMon%3==0){
						calender += "<tr>";
					}
					tbody = "<td width='33%' class='"
					+ (drawMonth==cMon?"ui-monthpicker-days-cell-over  ui-monthpicker-today":"") +"' data-event='click' data-handler='selectDay' data-year='"
					+ drawYear + "' data-month='"
						+ cMon + "'><a class='ui-state-default "
						+ (drawMonth==cMon?"ui-state-highlight":"") +"' href='#'>"
					+ monthNames[cMon] + "</a><td>";
					if(cMon%3==2){
						tbody += "</tr>";
					}
					calender += tbody;
				...
				calender += "</tbody></table>" + (isMultiMonth ? "</div>" +
							((numMonths[0] > 0 && col === numMonths[1]-1) ? "<div class='ui-monthpicker-row-break'></div>" : "") : "");
				group += calender;
			}
			html += group;
		}
	...
	_generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		var inMinYear, inMaxYear, month, years, thisYear, determineYear, year, endYear,
			...
			html = "<div class='ui-monthpicker-title'>";
		...
		if ( !inst.yearshtml ) {
			...
			if (secondary || !changeYear) {
				...
				inst.yearshtml += "<select class='ui-monthpicker-year' data-handler='selectYear' data-event='change'>";
...
function bindHover(dpDiv) {
	var selector = "button, .ui-monthpicker-prev, .ui-monthpicker-next, .ui-monthpicker-calendar td a";
	return dpDiv.delegate(selector, "mouseout", function() {
			$(this).removeClass("ui-state-hover");
			if (this.className.indexOf("ui-monthpicker-prev") !== -1) {
				$(this).removeClass("ui-monthpicker-prev-hover");
			}
			if (this.className.indexOf("ui-monthpicker-next") !== -1) {
				$(this).removeClass("ui-monthpicker-next-hover");
			}
		})
		.delegate(selector, "mouseover", function(){
			if (!$.monthPicker._isDisabledMonthPicker( instActive.inline ? dpDiv.parent()[0] : instActive.input[0])) {
				$(this).parents(".ui-monthpicker-calendar").find("a").removeClass("ui-state-hover");
				$(this).addClass("ui-state-hover");
				if (this.className.indexOf("ui-monthpicker-prev") !== -1) {
					$(this).addClass("ui-monthpicker-prev-hover");
				}
				if (this.className.indexOf("ui-monthpicker-next") !== -1) {
					$(this).addClass("ui-monthpicker-next-hover");
				}
			}
		});
}
