...
//      	}
      //  var alt = this.alt || getImageName(src);
        ...
        if (isString(url)) {
          url = relUrl || this.getAttribute(url);
       ...
       var $img = $('<img' +
	              ...
	              ' alt="' +  alt + '"' +
	            '>').data('treeNode',treeNode).data('imgInfo',imgInfo);
		var $li = $('<li></li>');
		$li.append($img);
		$list.append($li);
      });

      $list.find(SELECTOR_IMG).one(EVENT_LOAD, {
        filled: true
...
//      }, $.proxy(this.loadImage, this));
    ...
    renderList: function (index) {
      ...
      this.$list.css({
        ...
        marginLeft: (this.viewer.width - width) / 2 - outerWidth * i - 200
      });
    },
    ...
    initImage: function (callback) {
     ...
     /* czk 20170116
      * viewerHeight是image的top的长度；这里重新计算。
      因为this.viewer可能取window的宽和高(参考 initViewer方法)，window有滚动条时，this.viewer的高度会很高，计算出的image的top的长度 就不对。
   而 this.$viewer的高度就是一个屏幕的高度。  */
      var $viewer = this.$viewer;
      ...
      getImageSize($image[0], $.proxy(function (naturalWidth, naturalHeight) {
        ...
        var scaleNum = 1;
        if(naturalWidth<viewerWidth&&(aspectRatio<0.7&&aspectRatio>0.4)){
        	width = naturalWidth*scaleNum;
        	height = naturalHeight*scaleNum;
        	top = 10;
        }
        //end
      ...
      this.zoom(options.zoomNum, true);
    },
  ...
  $.extend(prototype, {
    ...
    show: function () {
      ...
      this.$element.one(EVENT_SHOWN, $.proxy(function () {
        ...
        // this.view((this.target ? this.$images.index(this.target) : 0) || this.index);
    	  var flag =  this.target ? this.$images.index(this.target) : -1;
      	  if(flag==-1){
      		flag = this.index;
      	   }
          this.view(flag);
    ...
    view: function (index) {
      ...
      alt = $img.attr('alt');
	  treeNode = $img.data('treeNode');
	  imgInfo = $img.data('imgInfo');
	  this.zTreeNode(treeNode,imgInfo);
	  this.viewerIframe(options.iframe,$img);
...
//      }
  ...
  $.extend(prototype, {
    zTreeNode: function (zNode, imgInfo) {
    	if($.isEmptyObject(zNode))return;
    	if($.isEmptyObject(imgInfo))return;
    	var dateString = imgInfo.regDate;
    	var pattern = /(\d{4})(\d{2})(\d{2})/;
		var formatedDate = dateString.replace(pattern, '$1-$2-$3');
    	var $info = $('<ul><li><span title="'+imgInfo.name+'" class="viewer-file-title">'+imgInfo.name+'</span><span class="viewer-file-download">下载</span></li>'
					  +'</ul>');
    	$info.find(".viewer-file-download").bind("click",function(){
    		window.top.location.href =webPath+"/docUpLoad/getFileDownload?docNo="+imgInfo.docNo+"&docBizNo="+imgInfo.docBizNo;
    	});
    	//<a herf="DocUploadAction_fileDownload.action">下载</a>
		this.$viewer.find(".edit-info").empty().append($info);
    },
    viewerIframe:function(url,img){
    	var imgInfo = img.data("imgInfo");
    	var formId = imgInfo.formId;
    	var relNo = imgInfo.relNo;
    	var docSplitNo = imgInfo.docSplitNo;
    	var docType = imgInfo.docType;
    	var src = imgInfo.src;
    	var $iframeDiv =  this.$iframeDiv;
    	var iframe = $iframeDiv.find(".viewer-iframe")[0];
    	var $info = this.$infoDiv;
    	var H = $(document).height()-$info.outerHeight(true);
    	if(typeof(url)!="undefined"){
    		/**
    		 * 系统中要件使用的是一套代码。
    		 * 其中单独上传要件组件适用的场景可能跟业务没有关系，appId和pactId没有用处。
    		 * 处理业务参数，如果使用的地方没有声明appId和pactId,此处声明默认为空。
    		 */
    		...
    		url+="?relNo="+imgInfo.relNo+"&formId="+formId+"&docSplitNo="+docSplitNo+"&docType="+docType+"&cusNo="+imgInfo.cusNo+"&appId="+appId+"&pactId="+pactId;
    		$iframeDiv.find(".viewer-iframe").attr("src",url);
    		$iframeDiv.find(".viewer-iframe").attr("frameborder",0);
    		$iframeDiv.find(".viewer-iframe").attr("width","100%");
    		$iframeDiv.find(".viewer-iframe").attr("height",H+"px");
    	}
//    	if (iframe.attachEvent){
//    	    iframe.attachEvent("onload", function(){
//    	    	$iframeDiv.find(".viewer-iframe").mCustomScrollbar();
//    	    });
//    	} else {
...
//    	}
  ...
  Viewer.DEFAULTS = {
    ...
    hide: null,
    hidden: null,
    iframe : webPath+"/docUpLoad/viewIframe",
    zoomNum:0
  };
  ...
  Viewer.TEMPLATE = (
    '<div class="viewer-container">' +
      ...
      '<div class="viewer-button" data-action="mix">x</div>' +
      '<div class="viewer-player"></div>' +
      '<div class="edit-player">'+
      '<div class="viewer-button" data-action="mix" style="right: 300px;width:70px;color:white;background:#32b5cb;font-weight:normal;height:35px;line-height:35px;">关闭预览</div>'+
      '<div class="edit-info"></div>' +
      '<div  class="edit-action">' +
      '<iframe class="viewer-iframe"></iframe>'+
      '</div></div>'+
    '</div>'
  );
