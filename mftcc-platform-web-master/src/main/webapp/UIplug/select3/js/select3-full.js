...
//	regArr.push(key);  
//}  
//var reUnescapedHtml = new RegExp('[' + regArr.join('') + ']', 'g');
...
Select3.Locale = {
    ...
    ajaxError: function(term) { return '有关 <b>' + escape(term) + '</b> 查询失败'; },
    loading: '正在加载...',
    loadMore: '加载更多...',
    needMoreCharacters: function(numCharacters) {
        return '输入 ' + numCharacters + '个以上字符进行查询';
    },
    noResults: '未找到结果',
    noResultsForTerm: function(term) { return '未找到有关  <b>' + escape(term) + '</b> 的信息'; }
...
Select3.Templates = {
    ...
    multipleSelectedItem: function(options) {
        ...
        return (
            '<span class="select3-multiple-selected-item' + extraClass + '" ' +
                  'data-item-id="' + escape(options.id) + '" name="'+escape(options.text)+'">' +
                escape(options.text) +
    ...
    resultItem: function(options) {
        return (
            '<div class="select3-result-item" data-item-id="' + escape(options.id) + '" name="'+escape(options.text)+'">' +
                escape(options.text) +
     ...
     * placeholder.字体图标由fa fa-sort-desc 改成了i i-jiantou7(因fa fa-sort-desc 字体是微软雅黑情况下不显示)
     */
    singleSelectInput: (
        '<div class="select3-single-select">' +
            '<div class="select3-single-result-container"></div>' +
            '<i class="i i-jiantou7 select3-caret"></i>' +
        '</div>'
    ),
