/*
 * $(function() {
	$('#mathStr').mathType({//事件初始化绑定点击事件
		mathRem: '#remark',//公式描述选择器，默认为空
		selectArr: selectArr,//公式参数，数据格式数组，例如：[{id: '{DATE}',text: '系統日期'},{id:'{REGNO}',text:'登记人编号'}]；
		extBtns: [{id: '{DATE}',text: '系統日期'},{id:'{REGNO}',text:'登记人编号'}]//扩展自定义按钮，数据格式，如公式参数一样
	})
});
 * */
(function(factory) {
	...
	function Viewer(element, options) {
		...
		this.options = $.extend({}, defaultOptions, $.isPlainObject(options) && options);
		this.isBuilt = false;
		$(element).bind("click", $.proxy(this.init, this));
	}
	$.extend(prototype, {
		init: function() {
			...
			this.extBtns = options.extBtns;
			this.opBtns = opBtns;
			this.expBtns = expBtns;
			this.$body = $('body');
			this.initVal = this.$element.val();
			this.build();
			$(document).bind("keydown", $.proxy(this.backspaceKey, this));
		}
	...
	$.extend(prototype, {
		build: function() {
			...
			this.$closeBtn = $viewer.find('.math-close-btn');
			this.$submitBtn = $viewer.find('.math-submit-btn');
		//	this.$select = $viewer.find('.math-select select');
			this.$select = $viewer.find('#selectExp');
		//	this.$insertBtn = $viewer.find('.math-select input');
			this.$mathStr = $viewer.find('.math-str textarea');
			this.$mathRem = $viewer.find('.math-rem textarea');
			this.$opBtns = $viewer.find('.math-ctrl .math-opBtns');
			this.$expBtns = $viewer.find('.math-ctrl .math-expBtns');
			this.$extBtns = $viewer.find('.math-ext .math-extBtns');
		...
		unbuild: function() {
			...
			this.$viewer.slideUp("slow",function(){
				$(this).remove();
			});
			this.isBuilt = false;
		}
	...
	$.extend(prototype, {
		ready: function() {
			this.$closeBtn.bind("click", $.proxy(this.unbuild, this));
		//	this.$insertBtn.bind("click", $.proxy(this.insertSelectVal, this));
			this.$submitBtn.bind("click", $.proxy(this.setExpVal, this));
			this.$mathStr.bind("keydown keyup blur",$.proxy(this.exp2str, this));
			this.addSelect();
			this.addBtn();
			this.addExtBtn();
			this.initExpVal();
...
//				$select.append($("<option value='" + selectArr[i].id + "'>" + selectArr[i].text + "(" + selectArr[i].id + ")" + "</option>").data("obj", selectArr[i]));
//			}
			for (var i = 0; i < selectArr.length; i++) {
				var opt = {};
				opt.id=selectArr[i].id;
				opt.text=selectArr[i].text + "(" + selectArr[i].id + ")";
				select3Arr.push(opt);
			}
			$select.select3({
				    allowClear: true,
				    items: select3Arr,
				    placeholder: '插入字段'
				}).on("select3-selected",$.proxy(this.insertSelect3Val, this));
		},
		addBtn: function() {
			var $opBtns = this.$opBtns;
			var opBtnArr = this.opBtns;
			var $expBtns = this.$expBtns;
			var expBtnArr = this.expBtns;
			var btnList = [];
			var expBtnList = [];
            var i,className,btn;
			for (i = 0; i < opBtnArr.length; i++) {
				className = '';
				if (typeof(opBtnArr[i].col)!="undefined") {
					className+=' btn_col_'+opBtnArr[i].col;
				}
				if (typeof(opBtnArr[i].row)!="undefined") {
					className+=' btn_row_'+opBtnArr[i].row;
				}
				btn = '<button data-val="' + opBtnArr[i].id + '" class="mathType_btn '+className+'">' + opBtnArr[i].text + '</button>'
				btnList.push(btn);
			}
			$opBtns.html(btnList.join(''));
			$opBtns.find('button').bind("click", $.proxy(this.editExp, this));
			for (i = 0; i < expBtnArr.length; i++) {
				className = '';
				if (typeof(expBtnArr[i].col)!="undefined") {
					className+=' btn_col_'+expBtnArr[i].col;
				}
				if (typeof(expBtnArr[i].row)!="undefined") {
					className+=' btn_row_'+expBtnArr[i].row;
				}
				btn = '<button data-val="' + expBtnArr[i].id + '" class="mathType_btn '+className+'">' + expBtnArr[i].text + '</button>'
				expBtnList.push(btn);
			}
			$expBtns.html(expBtnList.join(''));
			$expBtns.find('button').bind("click", $.proxy(this.editExp, this));
		},
		addExtBtn: function() {
			var $extBtns = this.$extBtns;
			var extBtnArr = this.extBtns;
			var $viewer  = this.$viewer;
			var btnList = [];
			if (!(extBtnArr.length>0)) {
				$viewer.css("width",$viewer.width()-200);
			}
			for (var i = 0; i < extBtnArr.length; i++) {
				var className = '';
				if (typeof(extBtnArr[i].col)!="undefined") {
					className+=' btn_col_'+extBtnArr[i].col;
				}
				if (typeof(extBtnArr[i].row)!="undefined") {
					className+=' btn_row_'+extBtnArr[i].row;
				}
				var btn = '<button data-val="' + extBtnArr[i].id + '" class="mathType_btn '+className+'">' + extBtnArr[i].text + '</button>'
				btnList.push(btn);
			}
			$extBtns.append(btnList.join(''));
			$extBtns.find('button').bind("click", $.proxy(this.editExp, this));
		}
	});
	$.extend(prototype, {
		initExpVal:function(){
			var mathStr = this.$mathStr;
			var initVal = this.initVal;
			mathStr.val(initVal);
			this.exp2str();
		},
		insertSelectVal:function(){
			var mathStr = this.$mathStr;
			var selectObj = this.$select;
			mathStr.val(mathStr.val() + $(selectObj).val());
			this.exp2str();
		},
		insertSelect3Val:function(e){
			//console.log($(this).select3('data'));
			var mathStr = this.$mathStr;
			mathStr.val(mathStr.val() + $(e.target).select3('data').id);
			this.exp2str();
		},
		editExp: function(e) {
			var mathStr = this.$mathStr;
			var mathRem = this.$mathRem;
			var btn = e.target;
			var $btn = $(btn);
			if ($btn.data('val') != "Del") {
				mathStr.val(mathStr.val() + $btn.data('val'));
			} else {
				this.delStr(mathStr.val());
			}
			this.exp2str();
		},
		...
		delStr: function(s) {
			...
			for (i = 0; i < selectArr.length; i++) {
				if (s.lastIndexOf(selectArr[i].id)!=-1&&(selectArr[i].id.length + s.lastIndexOf(selectArr[i].id)) == len) {
					s = s.substring(0, s.lastIndexOf(selectArr[i].id));
					flag = true;
				}
			}
			for (i = 0; i < extBtns.length; i++) {
				if (s.lastIndexOf(extBtns[i].id)!=-1&&(extBtns[i].id.length + s.lastIndexOf(extBtns[i].id)) == len) {
					s = s.substring(0, s.lastIndexOf(extBtns[i].id));
					flag = true;
				}
			}
			if (!flag) {
		...
		exp2str:function(){
            ...
            var i;
			for (i = 0; i < selectArr.length; i++) {
				s=s.replace(new RegExp(selectArr[i].id, "g"), selectArr[i].text);
			}
			for (i = 0; i < extBtns.length; i++) {
				s=s.replace(new RegExp(extBtns[i].id, "g"), extBtns[i].text);
			}
			$mathRem.val(s);
		}
	});
	$.extend(prototype, {
		setExpVal:function(){
			var $mathStr = this.$mathStr;
			var $mathRem = this.$mathRem;
			this.$element.val($mathStr.val());
			var mathRem = this.options.mathRem;
			if ($(mathRem).length>0) {
				$(mathRem).val($mathRem.val());
			}
			this.unbuild();
		}
	});
	...
	Viewer.TEMPLATE = ('<div class="math-body">' +
		'<div class="math-title">' +
		'<span>公式编辑器</span>' +
		'</div>' +
		'<div class="math-space math-edit">' +
		'<div class="math-select">' +
		'<div id="selectExp"></div>'+
		/*'<select>' +
		'<option></option>' +
		'</select>' +
		...
		'</div>' +
		'<div class="math-rem">' +
		'<textarea name="" rows="" cols="" readonly="readonly" disabled="disabled"></textarea>' +
		'</div>' +
		'</div>' +
		'<div class="math-space math-ctrl">' +
		'<div class="math-opBtns"></div>' +
		'<div class="math-expBtns"></div>' +
		'</div>' +
		'<div class="math-space math-ext">' +
		'<div class="math-extBtns">' +
		'</div>	' +
		'</div>' +
		'<div class="math-foot">' +
		'<button class="math-submit-btn">确认</button>' +
		'<button class="math-close-btn">取消</button>' +
		'</div>' +
		'</div>'
	);


	$.fn.mathType = function(options) {
		var $this = $(this);
