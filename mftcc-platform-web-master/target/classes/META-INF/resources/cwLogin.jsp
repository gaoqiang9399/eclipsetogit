...
<html>
  <head>
	...
	<meta http-equiv="description" content="金融业务系统">
...
<%-- 	<script src="${webPath}/themes/login/js/jquery.textillate.js"></script> --%>
	...
	<script type='text/javascript'src='${webPath}/dwr/interface/SysUserDwr.js'></script>
	<style type="text/css">
 ...
 /* function logoTextChange(){
    ...
    $(function(){
	    ...
	    //logoTextChange();
		 <s:if test="hasActionErrors()">
	      ...
	      if(actionmsg!=""){
                ...
                var opNo = "<s:property value = 'opNo'/>";
                var passwordhash = "<s:property value = 'passwordhash'/>";
                if($.trim(actionmsg).indexOf("密码错误")!=-1){
                	setTimeout("animateDom('.PwdInputDiv')",500);
                	$("#password-error").text(actionmsg);
                	$("#IdInput").val(opNo);
                	                	                  
					$("#PwdInput").val(passwordhash);
                }else{
                	$("#opname-error").text(actionmsg); 
                	setTimeout("animateDom('.IdInputDiv')",500);
                	$("#IdInput").val(opNo);
       ...
       /*   $.ajax({
     		url:'MfSysCompanyMstActionAjax_getByIdAjax.action',
     		success:function(data){
     			if(data.flag=="success"){
     				var srcWx=getImg(data.data.loginPageLogoImg);
 	...
 	function getImg(fileName){//初始化使用
 		...
 		if(fileName==""||fileName==null){//还未上传图片
 			...
 			src=webPath+"/MfSysCompanyMstAction_viewImage.action?uploadFileName="+fileName+"&type=loginLogo&rundom="+Math.random();;//返回流
 		}
 		return src;
 	}
    ...
    <div class="col-md-4 col-sm-6 col-xs-10" style="margin: 8% auto 0 auto; min-width: 460px;">
	    <div style="margin:7% auto;text-align: center;"> 
		    <div class="logo-div" style="margin-bottom: 35px;">
		    	<img src = "${webPath}/<%= mfSysCompanyMst.getLoginPageLogoImg() %>">
		    ...
		    <div class="loginFormDiv" style="margin: 0 15px;padding:35px; background-color: #109AA4;height: 10px; display: none;">
				<div class="login-div" style="">
					<form action="cms.action" name="LoginForm" method="post">
						<div class="form-group IdInputDiv" style="position: relative;margin-bottom: 0px;">
							<input type="text" class="form-control" style="background: #0A7279;border: none;color: #fff;height: 40px;padding-left: 34px;" placeholder="账户" id="IdInput" name="opName">
							<span class="glyphicon glyphicon-user form-control-feedback" style="left:0px;top:4px;font-size:16px;color:white;"></span>
							<input name="opNo" type="hidden" id="opNo" value="0000"/>
							<div style="height: 20px;color: #f34d00;" id="opname-error" class="error-div"></div>
						</div>
						<div class="form-group IdInputDiv" style="position: relative;margin-bottom: 0px;">
							<select type="text" class="form-control" style="background: #0A7279;border: none;color: #fff;height: 40px;padding-left: 34px;" placeholder="帐套名称" id="ztName" name="ztName">
								<!-- <option>基础帐套</option> -->
							</select>
							<span class="glyphicon glyphicon-list-alt form-control-feedback" style="left:0px;top:4px;font-size:16px;color:white;"></span>
							<input name="ztno" type="hidden" id="ztno" value="0000"/>
							<div style="height: 20px;color: #f34d00;" id="ztName-error" class="error-div"></div>
						</div>
						
						<div class="form-group PwdInputDiv" style="position: relative;margin-bottom: 0px;">
							<input type="password" class="form-control"  style="background: #0A7279;border: none;color: #fff;height: 40px;padding-left: 34px;"  placeholder="密码" id="PwdInput" name="passwordhash"> 
						...
						<input type="hidden" value="cwView"   id="sysFrame" name="frame"/>
					</form>
	...
	function doChkLogon () {
		...
		var ztName= document.getElementById('ztName').value;
		...
		if(!ztName || ztName==""){
			/* $("#loginInfo").css("display","block");
			$("#loginInfo").text("请输入用户名!"); */
			$("#ztName-error").text("没有可选择的帐套!");
			document.getElementById('ztName').focus();
			animateDom(".IdInputDiv");
			return false;
		}
		$("#opname-error").text("");
	...
	//------------------初始化-----------------------
        ...
        function checkExist(){
        	...
        	var isRepeat = false;
      	  	var isExists = false;
			dwr.engine.setAsync(false);
			SysUserDwr.checkExist($("#IdInput").val(),function(data) {
				if(data!=null&&data!=""){
					//console.log("--====----"+data);
					if(data.length>1){
						isRepeat = true;
						...
						$("#IdInput").val(data[0].opName);
			   			$("#opNo").val(data[0].opNo);
			   			getZtBooks();
			   			isExists = true;
        ...
        function  getZtBooks(){
	    	...
	    	$("#ztName").empty();
        	dwr.engine.setAsync(false);
    		SysUserDwr.getUserBooksList(userno,function(data){
	    		if(data!=null){
	    			var ztarr = data;
	    			//console.log("sds="+data);
	    			for ( var i = 0; i < ztarr.length; i++) {
							$("#ztName").append(
									"<option value="+ztarr[i].finBooks+">"
											+ ztarr[i].bookName + "</option>");
						} 
	    		}else{
	    			//obj.value =null;
	    		}
