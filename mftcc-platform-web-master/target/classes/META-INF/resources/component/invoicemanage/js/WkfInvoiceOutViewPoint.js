;
var WkfInvoiceOutViewPoint = function (window,$){
	var _init = function(){
	...
	var _doSubmitAjax = function(formObj){
		...
		if (flag) {
			var opinionType = $("input[name=approveResult]").val();//下拉框换成选择组件后，直接从input中取值
			//没有选择审批意见默认同意
			if(opinionType == undefined || opinionType == ''){
				opinionType = "同意";
			}
			//审批意见类型opinionType必须传，否则影响后续commitProcess方法中的if判断
			commitProcess(webPath+"/mfBusInvoicemanage/submitUpdateOutAjax?appNo="+invoiceoutId+"&opinionType="+opinionType,formObj,'mfBusinvoicecheck');
		}
	};
	//返回详情页面
