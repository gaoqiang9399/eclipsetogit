...
//MTU
  ...
  $.extend(prototype, {
    zTreeNode: function (url, alt, id) {
    	var $info = $('<ul><li><span title="'+alt+'" class="viewer-file-title">'+alt+'</span><span class="viewer-file-download">下载</span></li>'
					  +'</ul>');
    	$info.find(".viewer-file-download").bind("click",function(){
    		window.top.location.href = webPath+"/archiveInfoDetail/downloadFile?archiveDetailNo="+id;
    	});
    	//<a herf=webPath+"/docUpload/fileDownload">下载</a>
    ...
    viewerIframe:function(url,img){
    	...
    	if(typeof(url)!="undefined"){
    		//url+="?relNo="+imgInfo.relNo+"&formId="+formId;
    		url += "?archiveDetailNo=" + img.attr("id");
...
//    	}
  ...
  Viewer.DEFAULTS = {
    ...
    hide: null,
    hidden: null,
    iframe : webPath+"/archiveInfoDetail/getMainDetailPage",
    zoomNum:0
  };
