;
var perfectBusAppFee= function(window,$){
	var _init = function(){
		...
		if(itemNo=="3"){//如果时存出保证金时，费用主体取资金机构
			$("input[name=feeMainNo]").popupSelection({//费用主体
		...
		//加载收费账号选择组件
		var feeAccountId = $("input[name=feeAccountId]").val();
		if(feeAccountId!=""&&feeAccountId!=null){
			$("input[name=feeAccountId]").popupSelection({//账号选择
				searchOn : true, // false-不启用搜索，true-启用搜索
				inline : true, // true-内联,false-弹出
		...
		//加载关联合同选择组件
		$("input[name=relationPactNo]").popupSelection({//账号选择
			searchOn : true, // false-不启用搜索，true-启用搜索
			inline : true, // true-内联,false-弹出
			ajaxUrl : webPath+"/mfCusCreditContractDetail/getChildPactListDataByAppId?appId="+appId,
			//items:data.items,
			multiple : false, // false-单选,true-复选
		});
	};
