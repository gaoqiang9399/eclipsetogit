		...
		var legalIdType;
			$(function(){
			    myCustomScrollbar({
			    	...
			    	url:webPath+"/mfCusCorpBaseInfo/findByPageAjax?",//列表数据查询的url
			    	tableId:"tablecusIdNum00001",//列表数据查询的table编号
			    	tableType:"thirdTableTag",//table所需解析标签的种类
			    	pageSize:30,//加载默认行数(不填为系统默认行数)
			    	data:{legalIdType:"0"},//指定参数
			    	callback:function(){
				...
				$.ajax({
					...
					success:function(data){
						if(data.flag == "success"){
							parent.dialog.get('CusIdNumDialog').close(data.cusInfo).remove();
						}else{
							alert("获取客户信息失败");
						}
					},error:function(){
