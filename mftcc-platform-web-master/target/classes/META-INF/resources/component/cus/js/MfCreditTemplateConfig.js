;
var MfCreditTemplateConfig=function(window, $){
	//初始化产品基础配置信息
	...
	var getPrdctTemplateConfigHtml = function(data){
		...
		+'<button onclick="MfSysCreditConfig11.addBusNode(\'' + mfSysKind.busModel + '\',\'template\');" class="margin_left_15  btn add-node pointer">节点定制</button>'
		+'</div>'
	...
	var  getPrdctTemplateContentHtml = function(nodeList,nodeConfigMap, mfSysKind){
        ...
        $.each(nodeList, function (i, kindNode) {
            ...
            if (isExistDoc == "1") {
                ...
                $button.attr("onclick", "MfSysCreditConfig11.addBusNode('" + mfSysKind.busModel + "','template','" + nodeNo + "','" + mainApprove + "');");
	...
	var getNodeTemplateContentHtml = function(templateList,nodeNo){
		...
		$.each(templateList,function(i,kindNodeTemplate){
			...
			if(kindNodeTemplate.ifMustInput == "1"){//必填
                spanStr=spanStr+'<span style="background: #85B7EE;"><input class="pointer" onclick="MfCreditTemplateConfig.changeIfMustInput(this);" type="checkbox" checked="true" >必填</span>'
			}else{
                spanStr=spanStr+'<span style="background: #85B7EE;"><input class="pointer" onclick="MfCreditTemplateConfig.changeIfMustInput(this);" type="checkbox">必填</span>'
			}
			if(kindNodeTemplate.optPower == "2"){//可签
                spanStr=spanStr+'<span style="background: #5EB3C7;margin-left: 1px;"><input class="pointer" onclick="MfCreditTemplateConfig.changeOptPower(this);" type="checkbox" checked="true">可签</span>'
			}else{
                spanStr=spanStr+'<span style="background: #5EB3C7;margin-left: 1px;"><input class="pointer" onclick="MfCreditTemplateConfig.changeOptPower(this);" type="checkbox"  >可签</span>'
			}
			...
			+'<a href="javascript:void(0);" onclick="MfCreditTemplateConfig.templateSet(\''+kindNodeTemplate.ext2+'\',\''+kindNodeTemplate.ext3+'\');" class="pointer">设置模板</a>'
			+'</div>'
			+'</div>'
			+'<i class="i i-x5 item-del pointer" onclick="MfCreditTemplateConfig.deleteNodeTemplate(this,\''+nodeNo+'\',\''+kindNodeTemplate.kindNodeTemplateId+'\');"></i>'
			+'</div>';
		});
		//添加按钮
		htmlStr = htmlStr+'<div class="add-item-div" onclick="MfCreditTemplateConfig.addNodeTemplate(this,\''+nodeNo+'\');">'
		+'<div class="add-item">'
		+'<div class="item">'
		+'</div>'
		+'</div>'
	...
	var getTemplateContentHtml = function(nodeNo,mfTemplateModelConfig,mfKindNodeTemplate){
		var htmlStr = "";
		var inputStr="";
		var spanStr="";
		var imgClass = "i-word";
		if(mfTemplateModelConfig.templateSuffix=="2"){
			imgClass = "i-excel";
		}else if(mfTemplateModelConfig.templateSuffix=="3"){
			imgClass = "i-pdf-1";
		}else if(mfTemplateModelConfig.templateSuffix=="4"){
            imgClass = "i-icon_html";
        }
		...
		htmlStr = htmlStr+'<div class="block-item" data-ifmustinput="'+mfKindNodeTemplate.ifMustInput+'" data-optpower= "'+mfKindNodeTemplate.optPower+'" data-templateno="'+mfTemplateModelConfig.templateNo+'">'
		+'<div class="checkbox-div">'
		+ spanStr
		+'</div>'
		+'<div class="item">'
		+'<i class="i '+imgClass+' item-icon" ></i><span title="'+mfTemplateModelConfig.templateNameZh+'">'+mfTemplateModelConfig.templateNameZh+'</span>'
		+'</div>'
		+'<div class="hover-div pointer">'
		+'<div class="hover-info">'
		+'<a href="javascript:void(0);" onclick="MfCreditTemplateConfig.templateSet(\''+mfTemplateModelConfig.id+'\',\''+mfTemplateModelConfig.templateNameEn+'\');" class="pointer">设置模板</a>'
		+'</div>'
		+'</div>'
		+'<i class="i i-x5 item-del pointer" onclick="MfCreditTemplateConfig.deleteNodeTemplate(this,\''+nodeNo+'\',\''+mfKindNodeTemplate.kindNodeTemplateId+'\');"></i>'
		+'</div>';
			...
			success:function(data){
				if(data.flag=="success"){
					...
					if(fileNameOnly=="templateFile"){
						saveFileName = templateNo+fileNameEnd;
