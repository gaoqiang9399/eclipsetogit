...
var FullCalendar_month_list = function(window, $) {
	var _init = function () {
		$('#calendar').fullCalendar({
            ...
            events:function(start, end, callback){
					 ...
					 async: false,
					 cache:false,
		             url: webPath+"/sysLegalHolidayRecord/fullCalendarmonthlistAjax",
		             dataType: "json",
			...
			dayClick: function (date, allDay, jsEvent, view) {
			 ...
			 eventClick: function(calEvent, jsEvent, view) {
				 	  ...
				 	  //获取假日名称 开始事件 结束时间
				 	  var holidayName = calEvent.title;
				 	  var startDay = calEvent.start;
				 	  var endDay = calEvent.end;
				 	  
                      LoadingAnimate.start();
                      $.ajax({
                         url:webPath+"/sysLegalHolidayRecord/getByIdAjax",
                         type:"post",
                         /*
	...
	var _bindInsertClick = function() {
		...
		$(".insertAjax").click(function (){
			artdialog.close();
	          /*拼时间戳  */
			  var eventNew = new Object();
	          var startstr = $("#startdate").val(); //开始时间
	          var endstr = $("#enddate").val(); //结束时间
...
//	          eventNew.id = $("input[name='calendarNo']").val();//id
//	          eventNew.title = $("#title").val();//内容
//	          eventNew.start = startDate;//开始时间
//	          eventNew.end = endDate;//结束时间
//	          eventNew.className = $("#className").val();//类别
//	          $('#calendar').fullCalendar('renderEvent',eventNew,true);
			  //调整表单数据，并提交到底层
			 // $("input[name='eventTime']").val($("#starthour").val()+":"+$("#startmin").val());
	          ...
	          var url=webPath+"/sysLegalHolidayRecord/fullCalendarInsertAjax";
	          if(flag == "update-event"){//更新操作
	        	  url = webPath+"/sysLegalHolidayRecord/fullCalendarUpdateAjax";
	          }
	...
	var _deleteEvent = function(calen){
		artdialog.close();
		window.top.alert("是否确认删除该日程吗!",2,function(){
			LoadingAnimate.start();
			$.ajax({
				url:webPath+"/sysLegalHolidayRecord/fullCalendarDeleteAjax",
				data:calen,
				success:function(data){
					LoadingAnimate.stop();
					if(data.flag=="success"){
						//重新加载页面
						window.location.href=webPath+"/sysLegalHolidayRecord/goCalendar";
						alert(data.msg,1);
					}else{
						alert(data.msg,0);
					}
