		...
		$.each(
			{
				...
				click: function(seg, ev) {
					...
					var dblclick_event = function(e){
						view.trigger('eventClick', ca, seg.event, ev);
					}
					var obj = ev.target;
  					$(obj).unbind();
   					$(obj).skygqOneDblClick({oneclick:click_event,dblclick:dblclick_event});
					//view.trigger('eventClick', this, seg.event, ev);
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		titleHtml =
			'<span class="fc-title">' +'<i class="i i-fangdian fc-day-cude"></i>'+
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' +
					...
					(event.title ?
						'<div class="fc-title">' +'<i class="i i-fangdian fc-day-cude"></i>'+
...
var View = FC.View = Class.extend({
	...
	triggerDayClick: function(span, dayEl, ev) {
		...
		ca.trigger(
			'dayClick',
			dayEl,
			ca.calendar.applyTimezone(span.start), // convert to calendar's timezone for external API
			ev
		);
   			};
  		var obj = ev.target;
