;
var MfOaOpening_Confirm = function (window,$){
	var _init = function(){
...
//		});
		...
		$("input[name=openNumber]").on('keyup input',function(){
	...
	var _getBankByCardNumber = function(obj){
		...
		$.ajax({
			...
			success:function(data){
				if(data.flag == "success"){
					$("input[name=openBankNo]").val(data.bankId);
					$("input[name=openBandName]").val(data.bankName);
				}else{
