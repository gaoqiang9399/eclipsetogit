;
var MfOaAdjustment_Insert = function(window, $) {
	var _init = function() {
	};
	...
	var _setSysOrgInfo = function(sysOrg){
		$("input[name=newBrName]").val(sysOrg.brName);
		$("input[name=newBrNo]").val(sysOrg.brNo);
	};
	var _selectName = function(callback){
		...
		dialog({
			id:"selectNameDialog",
			title:'选择职员',
			url: webPath+"/mfOaArchivesBase/getNameSelect?opSts=1",
			width:800,
			height: tmpheight,
			backdropOpacity:0,
			onshow:function(){
