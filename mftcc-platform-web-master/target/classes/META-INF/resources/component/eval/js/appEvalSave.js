window.evalAjaxSave = function(obj,url){
	...
	if(flag){
		...
		var accountId = $(obj).find("input[name='accountId']").val();
		jQuery.ajax({
			url:ajaxUrl,
			data:{ajaxData:dataParam,cusBaseType:cusBaseType,timeLimit:timeLimit,useType:useType,gradeType:gradeType,evalClass:evalClass,accountId:accountId},
			type:"POST",
			dataType:"json",
			beforeSend:function(){  
				...
				if(data.flag == "success"){
					...
					AppEval_InitiateApp.initThead("",showData);
					if(!updateFlag){
						initData(data.listData);

						$.each(data.gradeCardListData,function(i,obj) {
							if (data["codeValue" + obj.gradeCardId]) {
								var str = "codeValue" + obj.gradeCardId;
								var dxList = data[str].scoreList;
...
window.refreshGradeCard = function(gradeCardListData){
	...
	$.each(gradeCardListData,function(i,obj){
		...
		divName=obj.gradeCardId;
		dataTarget="evalIndexDetailInfo"+obj.gradeCardId;
		tableHtml='<form id="evalIndex'+obj.gradeCardId+'" name="'+obj.gradeCardId+'">'+tableHtml+'</form>';
...
window.evalUpdate = function(){
	var dataParam="";
	debugger;
	$("form[id^=evalIndex]").each(function(i,obj){
		var gradeCardId=$(obj).attr("name");
	...
	jQuery.ajax({
		...
		beforeSend:function(){  
			...
			if(data.flag == "success"){
				...
				AppEval_InitiateApp.getEvalAdjDetailInfo();
				LoadingAnimate.stop();
	...
	jQuery.ajax({
		...
		beforeSend:function(){  
			...
			if(data.flag == "success"){
				...
				$(obj).removeAttr("action");
				AppEval_InitiateApp.getEvalAdjDetailInfo(obj);
				LoadingAnimate.stop();
				$(".showprogress ul").find("li[name=evalapp]").click();
			}else if(data.flag=="error"){
				alert(data.msg,0);
...
window.evalAppSubmit = function(obj,url){
	...
	if(flag){
		...
		jQuery.ajax({
			url:url,
			data:{ajaxData:JSON.stringify($obj.serializeArray()),useType:useType},
			type:"POST",
			dataType:"json",
			beforeSend:function(){  
			},success:function(data){
