...
function history  (obj,url) {
    ...
    top.window.openBigForm( url, "合同清单历史", function () {
    });
};
...
//校验评估价值2（校验后计算抵质押率）
function validateEvalAmount(obj) {
    var num = obj.value;//获取当前字段的输入值
    num = num.replace(/,/g, '');
    if (num.length != 0) {
        var reg = /^\-[,,0-9]*.?[0-9]*$/;
        if (reg.test(num)) {
            ...
            return compareEvalAndConfirm('evalAmount', num);
        }
    }
};

//校验担保金额
function validateConfirmAmount(obj) {
    var num = obj.value;//获取当前字段的输入值
    num = num.replace(/,/g, '');
    if (num.length != 0) {
        var reg = /^\-[,,0-9]*.?[0-9]*$/;
        if (reg.test(num)) {
            ...
            return compareEvalAndConfirm('confirmAmount', num);
        }
    }
};

//比较评估价值和担保金额
function compareEvalAndConfirm(putType, num) {
    if (putType == 'evalAmount' && num.length != 0) {
        var confirmAmount = $("input[name=confirmAmount]").val().replace(/,/g, "");
        if (confirmAmount.length != 0) {
            confirmAmount = parseFloat(confirmAmount);
            num = parseFloat(num);
            if (num < confirmAmount) {
                alert("评估价值不能小于反担保金额", 0);
                return false;
            }
        }
    } else if (putType == 'confirmAmount' && num.length != 0) {
        var evalAmount = $("input[name=evalAmount]").val().replace(/,/g, "");
        if (evalAmount.length != 0) {
            evalAmount = parseFloat(evalAmount);
            num = parseFloat(num);
            if (evalAmount < num) {
                alert("反担保金额不能大于评估价值", 0);
                return false;
            }
        }
    }
...
var MfBusCollateralRelDetail = function (window, $) {
    var _init = function () {
    ...
    var _showCollateralBaseInfo = function () {
        jQuery.ajax({
            ...
            data: {appId: appId, relId: relId, collateralType: collateralType,busModel:busModel},
            type: "POST",
            dataType: "json",
            beforeSend: function () {
            }, success: function (data) {
                if (data.flag == "success") {
                    var collateralName,htmlContent,pledgeMethod,collateralNameCapture,releaseCollateralStr,htmlStr,checkPmsBizFlag;
                    $.each(data.htmlMap, function (index, obj) {
                        ...
                        if (vouType == "2") {//保证信息
                            ...
                            htmlContent = data.htmlMap[index];
                            ...
                            if(ifEditFlag == '1'){
                                releaseCollateralStr = '<button class="btn btn-link formAdd-btn" id="release' + collateralId + '" onclick="MfBusCollateralRelDetail.releaseCollateral(\'' + collateralId + '\',\'' + collateralName + '\');" title="删除">删除</button>';
                            }else{
                            ...
                            //拼接要件资料开始
                            $("#collateralBaseXin" + collateralId).append(_addPledgeDocInfo(collateralId));
                            _showPledgeDocInfo(collateralId);
                            //拼接要件资料结束

                            checkPmsBizFlag = BussNodePmsBiz.checkPmsBiz("cus_assure_credit_info");
                            if (checkPmsBizFlag) {
                        ...
                        } else if (vouType == '10') {//最高额担保合同
                            collateralName = bean.name;
                            htmlContent = data.htmlMap[index];
                            pledgeMethod = vouType;
                            collateralNameCapture = collateralName
                            if (collateralName.length > 20) {
                            ...
                            htmlStr = '<div class="dynamic-block" name="collateralBase' + collateralId + '">' +
                                ...
                                '<div id="rotate-body' + collateralId + '\"></div></div></div></div></div>';
                            $(".block-add").after(htmlStr);
                            htmlStr = '<div class="handle"><span class="sub-title" id="' + collateralId + '">最高额担保合同</span></div>';
                            htmlStr = '<div class="cover infoTilte form-table" id="mf_highGrtContract_info' + collateralId + '">' + htmlStr +
                                '<form class="form-margin" action="' + webPath + '/mfHighGuaranteeContract/updateAjaxByOne.action" id="MfHighGuaranteeContract_updateAjaxByOne">' + htmlContent + '</form></div>';
                            $("#rotate-body" + collateralId).before(htmlStr);
                        } else if (vouType != "2") {//抵质押信息
                            ...
                            var updateBill ='';
                           var classFirstNo =  bean.classFirstNo;
                           var extOstr33 =  bean.extOstr33;
                           var classTips="";
                            if(classFirstNo=='C'){
                                if(bean.extDic20=="1"){
                                    classTips= '<span class="formName" style="color: red" >应收账款余额小于应收账款抵押余值 </span>'
                                }
                                updateBill='<button class="btn btn-link formAdd-btn" id="updateBill' + collateralId + '" onclick="MfBusCollateralRelDetail.uploadBill(\'' + collateralId + '\');" title="上传合同清单">上传合同清单</button>';
                            }
                            //判断置换按钮权限是否开启
                            checkPmsBizFlag = BussNodePmsBiz.checkPmsBiz("pledge-replace");
                            if (!checkPmsBizFlag) {
                                replaceCollateral = "";
                            }
                            htmlStr = '<div class="dynamic-block" name="collateralBase' + collateralId + '">' +
                                '<div class="form-table"><div class="title" id="title' + collateralId + '">' +
                                '<span class="formName" title="' + collateralName + '"><i class="i i-xing blockDian"></i>' + collateralNameCapture + '(' + keepStatusName + ')' + '</span>' +classTips+
                                releaseCollateralStr + updateBill+inStockStr + outStockStr + replaceCollateral + lowestWorthAdjust +
                            ...
                            //拼接要件资料开始
                            $("#collateralBase" + collateralId).append(_addPledgeDocInfo(collateralId));
                            _showPledgeDocInfo(collateralId);
                            //拼接要件资料结束

                            //控制单个押品入库和出库操作------------------------------
                            //押品状态为登记、已使用、入库后的状态、放款状态未审批完成，不允许入库
                            ...
                            if (extOstr33 != "2" ) {
                                $("#updateBill" + collateralId).attr("disabled", "true").attr("class", "btn btn-link formAdd-btn set-disabled");
                            }
                    ...
                    if (checkPmsBizFlag) {
                        if (data.entrance != "" && data.entrance == "credit") {
                            ...
                            var htmlTableContentCusNo1 = '<div class="form-table">' + '<div class="title"><span class="sub-title">该客户所担保的保证信息</span> </div>' + '<div class="list-table-replan">' + '<div class="content margin_left_15 collapse in">'
                                ...
                                '</div>' + '</div>' + '</div>'
                            $("#pledgeBaseInfo").after(htmlTableContentCusNo1);
                        }
                        ;
                    }
                }
    ...
    var _addCollateralInfo = function () {
        var url;
        if (typeof(extensionApplyId) != "undefined" && extensionApplyId != "") {

            url = webPath + "/mfBusCollateralRel/insertInput?cusNo=" + cusNo + "&appId=" + relId + "&entrFlag=collateral&entrance=" + entrance + "&extensionApplyId=" + extensionApplyId;

        } else {

            url = webPath + "/mfBusCollateralRel/insertInput?cusNo=" + cusNo + "&appId=" + relId + "&entrFlag=collateral&entrance=" + entrance;
            ...
            top.collateralName = "";
            top.collateralType = "";
            top.openBigForm(url, "新增反担保信息", function () {
                if (top.addCollateral) {
                    ...
                    if (vouType == "2") {
                        var arr = new Array();
                        arr = collateralId.split('|');
                        var arr1 = new Array();
                        arr1 = collateralName.split('|');
                        for (var i = 0; i < arr.length; i++) {
                            collateralName = arr1[i];
                            jQuery.ajax({
                                url: webPath + "/mfBusCollateralRel/getBaseCollateralHtmlAjax?entrance=" + entrance + "&cusNo=" + cusNo,
                                data: {collateralId: arr[i], collateralType: collateralType, vouType: vouType, num: i,busModel:busModel},
                                type: "POST",
                                dataType: "json",
                                beforeSend: function () {
                                }, success: function (data) {
                                    ...
                                    if (data.flag == "success") {
                                        var i = data.num;
                                        var collateralNameCapture = arr1[i];
                                        if (arr1[i].length > 20) {
                                            collateralNameCapture = arr1[i].substring(0, 20) + "...";
                                        }
                                        if (vouType == "2") {//保证信息
                                            var releaseCollateralStr = '<button class="btn btn-link formAdd-btn" id="release' + arr[i] + '" onclick="MfBusCollateralRelDetail.releaseCollateral(\'' + arr[i] + '\',\'' + arr1[i] + '\');" title="删除">删除</button>';
                                            ...
                                            var htmlStr = '<div class="dynamic-block" name="collateralBase' + arr[i] + '">' +
                                                '<div class="form-table"><div class="title" id="title' + arr[i] + '">' +
                                                '<span class="formName" title="' + arr1[i] + '"><i class="i i-xing blockDian"></i>' + collateralNameCapture + '</span>' +
                                                releaseCollateralStr +
                                                '<button class=" btn btn-link pull-right formAdd-btn" data-toggle="collapse" data-target="#collateralBaseXin' + arr[i] + '">' +
                                                '<i class="i i-close-up"></i><i class="i i-open-down"></i></button></div>' +
                                                '<div id="collateralBaseXin' + arr[i] + '">' +
                                                '<div class="content collapse in" id="collateralBase' + arr[i] + '">' +
                                                '<div id="rotate-body' + arr[i] + '\"></div></div></div></div></div>';
                                            $(".block-add").after(htmlStr);

                                            htmlStr = '<div class="handle"><span class="sub-title" id="' + arr[i] + '">保证信息</span></div>';
                                            htmlStr = '<div class="cover infoTilte form-table" id="mf_assure_info' + arr[i] + '">' + htmlStr +
                                                '<form class="form-margin" action="/mfAssureInfo/updateAjaxByOne.action" id="MfAssureInfoAjax_updateByOne">' + data.htmlStr + '</form></div>';
                                            $("#rotate-body" + arr[i]).before(htmlStr);
                                            // if (data.entrance != "" && data.entrance == "credit") {
                                            //     var htmlTableContent = data.assureInfosStr;
                                            //     var htmlTableContent1 = '<div class="list-table-replan"  id="collateral' + arr[i] + '">' + '<div class="content margin_left_15 collapse in">'
                                            //         + htmlTableContent +
                                            //         '</div>' + '</div>' + '</div>'
                                            //     $("#collateralBase" + arr[i]).after(htmlTableContent1);
                                            // }
                                            //拼接要件资料开始
                                            $("#collateralBaseXin" + arr[i]).append(_addPledgeDocInfo(arr[i]));
                                            _showPledgeDocInfo(arr[i]);
                                            //拼接要件资料结束
                                            _updateCollateral();
                                            _switchShowHide();
                                        }
                        ...
                        jQuery.ajax({
                            ...
                            beforeSend: function () {
                                ...
                                if (data.flag == "success") {
                                    ...
                                    var htmlStr,releaseCollateralStr;

                                    if (vouType == '10') {//最高额担保合同
                                        releaseCollateralStr = '<button class="btn btn-link formAdd-btn" id="release' + collateralId + '" onclick="MfBusCollateralRelDetail.releaseCollateral(\'' + collateralId + '\',\'' + collateralName + '\');" title="删除">删除</button>';
                                        htmlStr = '<div class="dynamic-block" name="collateralBase' + collateralId + '">' +
                                            '<div class="form-table"><div class="title" id="title' + collateralId + '">' +
                                            '<span class="formName" title="' + collateralName + '"><i class="i i-xing blockDian"></i>' + collateralNameCapture + '</span>' +
                                        ...
                                        htmlStr = '<div class="handle"><span class="sub-title" id="' + collateralId + '">最高额担保合同</span></div>';
                                        htmlStr = '<div class="cover infoTilte form-table" id="mf_highGrtContract_info' + collateralId + '">' + htmlStr +
                                            '<form class="form-margin" action="' + webPath + '/mfHighGuaranteeContract/updateAjaxByOne.action" id="MfHighGuaranteeContract_updateAjaxByOne">' + data.htmlStr + '</form></div>';
                                        $("#rotate-body" + collateralId).before(htmlStr);
                                        if (data.entrance != "" && data.entrance == "credit") {
                                            var htmlTableContent = data.assureInfosStr;
                                            var htmlTableContent1 = '<div class="list-table-replan"  id="collateral' + collateralId + '">' + '<div class="content margin_left_15 collapse in">'
                                        ...
                                        var lowestWorthAdjust = '<button class="btn btn-link formAdd-btn" id="worthAdjust' + collateralId + '" onclick="MfMoveableCommon.lowestWorthAdjust(\'' + collateralId + '\');" title="最低监管价值调整">监管调整</button>';
                                        //判断置换按钮权限是否开启
                                        var checkPmsBizFlag = BussNodePmsBiz.checkPmsBiz("pledge-replace");
                                        if (!checkPmsBizFlag) {
                                            replaceCollateral = "";
                                        }
                                        var updateBill="";
                                        var classFirstNo =  data.pledgeBaseInfo.classFirstNo;
                                        var extOstr33 =  data.pledgeBaseInfo.extOstr33;
                                        if(classFirstNo=='C'){
                                            updateBill='<button class="btn btn-link formAdd-btn" id="updateBill' + collateralId + '" onclick="MfBusCollateralRelDetail.uploadBill(\'' + collateralId + '\');" title="上传合同清单">上传合同清单</button>';
                                        }
                                        htmlStr = '<div class="dynamic-block" title="押品信息" name="collateralBase' + collateralId + '">' +
                    ...
                    top.window.openBigForm(webPath + '/mfBusCollateralRel/inStockBatchInput?appId=' + appId + "&fincId=" + fincId, "批量入库", function () {
                        if (top.inStockBatch) {
                            ...
                            if (top.isExamineInOutStock == "1") {
                                $("#batchInStock").find("span").html("入库审批中");
                                $.each(data.busCollateralDetailRelList, function (key, obj) {
                                    //$("button[id=inStock"+busCollateralDetailRel.collateral_id+"]").html("入库审批中");
                                    $("#inStock" + obj.collateralId).html("入库审批中");
                                });
        ...
        top.window.openBigForm(webPath + '/mfBusCollateralRel/outStockBatchInput?appId=' + appId + "&authCycle=" + authCycle + "&fincId=" + fincId, "批量出库", function () {
            if (top.outStockBatch) {
                ...
                if (top.isExamineInOutStock == "1") {//需要审批
                    $("#batchOutStock").find("span").html("出库审批中");
                    jQuery.ajax({
                        url: webPath + "/mfBusCollateralRel/getNotOutStockListAjax",
                        data: {appId: appId},
                        type: "POST",
                        dataType: "json",
                        beforeSend: function () {
                            ...
                            $.each(data.busCollateralDetailRelList, function (key, obj) {
                                if (obj.outSts != "2") {
                                    $("#outStock" + obj.collateralId).html("出库审批中");
                                }
                            });
                        }, error: function (data) {
    ...
    //拼接要件资料html
    var _addPledgeDocInfo = function (collateralId) {
        var pledgeDocInfoHtml = "";
        //列表形式展示要件
        if (docShowType == "1") {
            pledgeDocInfoHtml = '<div class="overflowHidden bg-white">'
                + '<div class="row clearfix"><div class="upload_body"><ul class="ztree" id="uploadTree' + collateralId + '">'
                + '<li id="uploadTree_1" class="level0" tabindex="0" hidefocus="true" treenode="">'
                + '<a id="uploadTree_1_a" class="level0" treenode_a="" onclick="" target="_blank" style="">'
                + '<span id="uploadTree_1_span" class="node_name"><i class="i i-xing blockDian"></i>要件列表</span>'
                + '<button title="新增" onclick="docManageAloneAdd(\'' + collateralId + '\');return false;" style="position: relative;display: inline-block;top: -3px;left: 2px;" class="btn btn-link formAdd-btnFile">'
                + '<i class="i i-jia3"></i></button>'
                + '<button class=" btn btn-link pull-right formAdd-btn" data-toggle="collapse" data-target=".doc-manage-table" aria-expanded="true">'
                ...
                + '<div id="content' + collateralId + '" class="table_content list-table doc-manage-table" style="height: auto;"></div></div>'
    ...
    //展示要件资料
    var _showPledgeDocInfo = function (collateralId) {
        dataDocParm = {
            relNo:collateralId,
            docType:"19",
            docTypeName: "反担保资料",
    ...
    //押品关联合作银行列表
    var _agenciesPledgeRelList = function () {
        //默认取授信号，业务场景授信号为空时取申请号
        var url = '/mfBusAgenciesPledgeRel/getListPage?appId=' + creditAppId;
        if(typeof creditAppId == "undefined" || creditAppId==""){
            url = '/mfBusAgenciesPledgeRel/getListPage?appId=' + appId;
        }
        top.window.openBigForm(webPath + url , "合作银行关联列表", function () {
        });
        //window.location.href=webPath+'/mfBusAgenciesPledgeRel/getListPage?appId=' + appId + "&creditAppId=" + creditAppId;
    };
    //上传合同清单
    var _uploadBill = function (collateralId) {
        var url = '/pledgeBaseInfo/uploadBill?appId=' + creditAppId;
        if(typeof creditAppId == "undefined" || creditAppId==""){
            url = '/pledgeBaseInfo/uploadBill?appId=' + appId;
        }
        top.window.openBigForm(webPath + url+ "&collateralId=" + collateralId, "上传合同清单", function () {
            ShowPledgeDetailInfo.addCollateralFormInfoCall(collateralId,'');
        });
    };
    return {
        init: _init,
        ...
        agenciesPledgeRelList:_agenciesPledgeRelList,
...
function dblUpdateVal(key, data) {
    ...
    if (key == "pledgeType") {
        if(typeof(relId) != "undefined" && typeof(relId) != null && relId != ''){
            data["relId"] =relId;
        }
