$(function(){
	...
	$(".select-border input[type=checkbox],.form_content input[type=checkbox]").rcSwitcher({
		width: 50,
		height: 18,
		theme: 'light',
		blobOffset: 1,
			...
			$(this).find("td").each(function(index,obj){
				...
				if(col.length>=index+1){
					...
					if(col[index]=="sts"){
						...
						$(obj).html(checkbox);
						//$(obj).children().rcSwitcher({width: 50,height: 18,theme: 'light',blobOffset: 1,onText:'是',offText:'否'});
...
function parmDicInput(obj,url){
	...
	if(keyName!=undefined){
		...
		$tableObj.find("tbody tr").eq(0).find("td input[type='checkbox']").rcSwitcher({width: 50,height: 18,theme: 'light',blobOffset: 1,onText:'是',offText:'否'});
		$(obj).parents(".col_content").find(".table_content").mCustomScrollbar("scrollTo","top");
	}else{
		//$.myAlert.Alert("请选择数据字典描述,然后新增！");
...
function intiPageShow($obj,pager){
	...
	if(flag===undefined){
		var pageShowHtml = '<div class="pagerShow">';
    		...
    		pageShowHtml+= '<span class="split"></span>已加载<span class="loadCount">'+(pager.pageSize*pager.pageNo)+'</span>条';
    		pageShowHtml+= '</span>';
    		pageShowHtml+= '</div>';
...
function ParmKeyInput(obj,url){
	...
	$formObj.find(".swraper .stoggler").removeClass("off").addClass("on").css({transform:"translateX(-9px)"});
}
//取消
...
function ParmKeySave(obj,url){
	...
	if(dataParam.keyName!=undefined&&dataParam.keyName!=""&&dataParam.keyChnName!=null){
		...
		if(dataParam.keyChnName==""){
			msg +=top.getMessage("NOT_FORM_EMPTY", "<br>数据字典名称:");
		}
		//$.myAlert.Alert(msg);
		alert(msg,0);
	}
		...
		success:function(data){
			if(data.flag == "success"){
				...
				if(data.formData.ifSts=="1"){
					...
					$form.find("[name='ifSts']").prev().find(".stoggler").removeClass("off").addClass("on").css({transform:"translateX(-9px)"});
				}else{
					$form.find("[name='ifSts']").prop('checked',false);
					$form.find("[name='ifSts']").prev().find(".stoggler").removeClass("on").addClass("off").css({transform:"translateX(-41px)"});
				}
				if(data.formData.ifEdit=="1"){
					$form.find("[name='ifEdit']").prop('checked',true);
					$form.find("[name='ifEdit']").prev().find(".stoggler").removeClass("off").addClass("on").css({transform:"translateX(-9px)"});
				}else{
					$form.find("[name='ifEdit']").prop('checked',false);
					$form.find("[name='ifEdit']").prev().find(".stoggler").removeClass("on").addClass("off").css({transform:"translateX(-41px)"});
				}
			}else{
				//$.myAlert.Alert(data.msg);
				alert(data.msg,0);
...
function selectParmKey(){
	...
	$.ajax({
		...
		data:{
			...
			//intiPageShow($(".table_content.child"),data.ipagec);
