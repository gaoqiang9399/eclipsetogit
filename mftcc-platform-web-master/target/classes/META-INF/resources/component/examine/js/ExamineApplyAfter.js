;
var ExamineApplyAfter=function(window,$){
	var showData = {
			dx:{//定性
				"indexName":"指标名称",
				"ctrl_btn":"打分选项"},
        ...
        dialog({
            ...
            onclose:function(){
                if(typeof(this.returnValue) == "undefined" || typeof(this.returnValue) == null || this.returnValue == ''){
                    ...
                    $("form[id=ecamHisInsertForm]").find("input[name=rptDate]").val(this.returnValue.rptDate);
                    $("form[id=ecamHisInsertForm]").find("input[name=rptType]").val(this.returnValue.rptType);
                    $("form[id=ecamHisInsertForm]").find("input[name=accountId]").val(this.returnValue.accountId);
                    $("form[id=ecamHisInsertForm]").find("input[name=rptDate]").blur();
                }
            }
	...
	//自动分级
    var _autoLevel=function(){
    	var cusNo =$("input[name='cusNo']").val();
        var parmData = {'cusNo': cusNo};
        $.ajax({
            url : webPath+"/mfExamineHis/autoLevel",
            type : "post",
            data : {ajaxData: JSON.stringify(parmData),tableId:"tablemfBusPactExamineList"},
            dataType : "json",
            success : function(data) {
                if (data.flag == "success") {
                    if (data.sideLevelFlag !== undefined && data.sideLevelFlag == '1') {
                        $("select[name=riskLevel]").val("2");
                        var riskLevel=  $("select[name=riskLevel]").val();
                        if(riskLevel==""){
                            $("select[name=riskLevel]").val("5");
						}
...
function getFinDataHtml(data){
    ...
    finDiv += '<th align="center" width="10%" scope="col" name="pushCapitalScale" sorttype="0">操作</th>'+
        ...
        '<tbody id="tab">';
    var htmlStr = "";
    if(data.cusFinMainList!=null&&data.cusFinMainList.length > 0){
        ...
        $.each(data.cusFinMainList,function(i,cusFinMain){
            ...
            if(data.cusType=="2") {  //个人客户时
                ...
                confirStr = webPath+"/cusFinMain/updateReportConfirmAjax?finRptType="+cusFinMain.reportType+"&finRptDate="+cusFinMain.weeks+"&cusNo="+cusFinMain.cusNo+"&relationCorpNo=" + cusFinMain.corpNo;
            }else{
                ...
                confirStr = webPath+"/cusFinMain/updateReportConfirmAjax?finRptType="+cusFinMain.reportType+"&finRptDate="+cusFinMain.weeks+"&cusNo="+cusFinMain.cusNo;
            }
