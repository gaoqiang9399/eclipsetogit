;
var ExamineApplySaveAfter=function(window,$){
            ...
            jQuery.ajax({
                ...
                beforeSend:function(){
                    ...
                    if(data.flag == "success"){
                        ...
                        window.location.href = webPath+"/mfExamineHis/examineApplyAfter?pasNo="+data.entityData.pasNo
    ...
    //保存第二步
    var _examSecondSave=function(){
        var $obj1 = $('#gradeCardInsertForm');
        var $obj2 = $('#gradeCardInsertForm2');
        var flag1 = submitJsMethod($obj1.get(0), '');
        var flag2 = submitJsMethod($obj2.get(0), '');
        if(flag1&&flag2){
            var ajaxUrl = $obj1.attr("action");
            var updateFlag = false;
            if(ajaxUrl===undefined||ajaxUrl==null||ajaxUrl==""){
                ajaxUrl = url;
                updateFlag = true;
            }
            var dataParam = JSON.stringify($obj1.serializeArray());
            var dataParam2 = JSON.stringify($obj2.serializeArray());
            LoadingAnimate.start();
            var dataParamJson = JSON.parse(dataParam);
            dataParam = JSON.stringify(dataParamJson);
            var dataParamJson2 = JSON.parse(dataParam2);
            dataParam2 = JSON.stringify(dataParamJson2);
            jQuery.ajax({
                url:ajaxUrl,
                data:{ajaxData:dataParam,ajaxData2:dataParam2},
                type:"POST",
                dataType:"json",
                beforeSend:function(){
                },success:function(data){
                    ...
                    if(data.flag == "success"){
                        ...
                        $("#addEval").remove();//去掉操作按钮，避免重复操作
                        $("#editEval").remove();//去掉操作按钮，避免重复操作
	...
	var _examApplySubmit=function(obj){
		var flag = submitJsMethod($(obj).get(0), '');
		if(flag){
			alert(top.getMessage("CONFIRM_OPERATION","保后检查提交"),2,function(){
	...
	return{
        ...
        examSecondSave:_examSecondSave,
	};
}(window,jQuery);