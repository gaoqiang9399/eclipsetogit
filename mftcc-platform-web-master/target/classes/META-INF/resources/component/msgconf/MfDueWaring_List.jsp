	...
	<head >
		...
		$(function(){
		    myCustomScrollbar({
		    	...
		    	url : webPath + "/cuslendWarning/findDueWarningByPageAjax", //列表数据查询的url
		    	tableId : "tableduewaringlist", //列表数据查询的table编号
        ...
        function baseTagSet(obj, url){
			...
			function addConfig(){
				top.openBigForm(webPath + "/cuslendWarning/getDueWarningInput","新增到期预警提醒",function(){
					window.updateTableData();
				});
			};
			function clickConfig(url){
				top.openBigForm(url,"到期预警提醒配置",function(){
					window.updateTableData();
				});
			};
