var TagItemList = function(window,$){
    var _init = function(){
        ...
        $.each(groupFlag,function(i,mapObj){
            itemsHtml += _setHtml(mapObj,itemList);
        });
    ...
    var _setHtml= function(mapObj,itemList){
        var type=mapObj.optCode;
        var optName=mapObj.optName;
        var subHtml="";
        var titleHtml='<div style="margin-left: 45%;"><span style="font-size: 18px;">'+optName+'</span></div>';
        var allCheck = titleHtml+"<input type='button' value='全选' onclick=\"TagItemList.allInCheck('type"+type+"')\"><input type='button' style='margin-left:5px' value='反选' onclick=\"TagItemList.backCheck('type"+type+"')\">";
    ...
    var _updateTagSet = function(){
        var templatePath = "/component/model/docmodel/";
        var tagkeyno = "";
        var tagkeyname="";
        ...
        jQuery.ajax({
            url:webPath+"/cuslendWarning/updateTagSetAjax",
