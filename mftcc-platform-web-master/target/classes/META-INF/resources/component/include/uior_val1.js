...
function func_uior_calAll(frm) {
	...
	if (errorMsg != '' && errorMsg != 'undefined') {
		if ($(".scroll-content.mCustomScrollbar").length > 0) {
			$(".scroll-content").mCustomScrollbar("scrollTo", $(frm).find(".error:eq(0)"));
		} else {
			 $('html, body').animate({  
                 scrollTop: $(frm).find(".error:eq(0)").offset().top  
             }, 500);  
		}
		return false;
	}
			...
			for ( var i = 0; i < funcs.length; i++) {
				...
				if (formula != null && formula != undefined && formula != '') {
					...
					if (!result) {
						...
						if(errorMsg!=''){
							errorMsg = errorMsg + "\n校验条件:[ "+curFormula+" ]不满足!";
							func_uior_addTips(obj, errorMsg);
						} else {
    ...
    // 要件必传验证
    if(!validateDocMustInput(frm)){
        return false;
    };

	//电子文档必填验证
	if(!validateTemplateMustInput(frm)){
        return false;
	};
...
// 要件必传验证
var validateDocMustInput = function(frm) {	//要件必填验证
    var flag=true;
    var len = $(frm).parents(".scroll-content").find(".btspan0").length;
    if(len>0){
        var docMsg="";
        $(frm).parents(".scroll-content").find(".btspan0").each(function(index){
...
// 要件必传验证
var validateTemplateMustInput = function(frm) {	//电子文档必填验证
    var flag=true;
    var len = $("#bizConfigs").length;
    if(len>0){
        var templateBizConfigIds="";
        $("#bizConfigs tbody tr").each(function(index){
        	var ifMustInput = $(this).find("input[name=ifMustInput]").val();
        	if(ifMustInput=="1"){
                templateBizConfigIds = templateBizConfigIds+$(this).find("input[type=hidden][name=templateBizConfigId]").val()+",";
            }
        });

        if(templateBizConfigIds!=""){
            $.ajax({
                url:webPath+ "/mfTemplateBizConfig/validateTemplateIsInput",
                data:{templateBizConfigIds:templateBizConfigIds},
                async:false,
				success: function(data) {
                    if(data.flag=="error"){
                        var msg = data.msg;
                        alert(top.getMessage("NO_SAVE",msg),0);
                        flag = false;
                    }
...
function func_uior_valForm(frm){
	...
	for(var i=0;i<frm.length;i++){
		...
		var oneMsg = func_uior_valTypeImm(obj);
		if (oneMsg) {
			func_uior_addTips(obj, oneMsg);
			msg = msg.concat(oneMsg);
		}
...
//		}
//		
//		var m1 = func_uior_valLength(obj);
//		msg=msg.concat(m1);
//		if (m1) {
//			func_uior_addTips(obj, m1);
//		}
//		var m2 = func_uior_valueFormat(obj);
//		msg=msg.concat(m2);
//		if (m2) {
//			func_uior_addTips(obj, m2);
//		}
...
function func_uior_isNum(obj){
	var str=obj.value;
	var msg='';
	var result=isNaN(parseFloat(str));
	if(!result){
...
function func_uior_valStringType(obj){
	...
	if(type!='undefined'&&func_uior_trim(type)!=''){	
		...
		obj.value="0";
			...
			if(str.length==10 || str.length==8 || str.length==19){
			 	var yy=0,mm=0,dd=0,hh=0,nn=0,ss=0;
	    		if(str.length==8){
	        		...
	        		mm = parseInt(str.substring(5,7), 10);
	        		dd = parseInt(str.substring(8,10), 10);
	        		hh = parseInt(str.substring(11,13), 10);
	        		nn = parseInt(str.substring(14,16), 10);
	        		ss = parseInt(str.substring(17,19), 10);
	    		}
	    		if(str.length==10 || str.length==8){
	    			if(mm > 12 || mm <= 0 || dd <= 0 || dd > 31 ) 
	        			return obj.getAttribute('title')+'中的内容不是日期!\n';
	    		}else{
	    			if(mm > 12 || mm <= 0 || dd <= 0 || dd > 31 || hh < 0 || hh >24 || nn < 0 || nn > 60 || ss < 0 || ss > 60) 
	        			return obj.getAttribute('title')+'中的内容不是日期!\n';
	    		}	
...
function func_uior_valTypeImm(obj){
	...
	var errorLabel = $(obj).data("errorLabel");
	if (errorLabel) {
		$(obj).removeData("errorLabel");
		errorLabel.remove();
	}
...
//用于时时验证数据类型,错误信息以alert弹窗提示
function func_uior_valTypeImm_alert(obj){
	var str=obj.value;
	var type=obj.getAttribute('datatype');
	if ($(obj).hasClass("Required")) {
		$(obj).removeClass("Required");
...
/**
 * 验证字段值的格式并增加tips，用于表单的onBlur事件。
 * @param obj this
 * @param type 用于保存验证方法中读取验证类型。必需。
 */
function func_uior_valFormat_tips(obj, type) {
	var msg = func_uior_valueFormat(obj);
	if(msg != ""){
		func_uior_addTips(obj, msg);
	}
}
/**
 * 用于动态切换-表单项格式验证类型。类型在func_uior_valueFormat中定义。
 * @param obj DOM对象
 * @param typeNew 要修改的验证类型，可以为空：为空时不做格式验证（去掉原有的func_uior_valFormat_tips函数）。
 * @returns {String}
 */
function changeValidateType (obj, typeNew) {
	/*
	 * 从onBlur事件中解析出验证类型。
	 * 兼容save动作和onBlur事件。
	 */
	var blur=obj.getAttribute('onblur');
	var newFunc = '';
	if (!!typeNew) {
		// 有值的情况下，确定新验证。
		newFunc = "func_uior_valFormat_tips(this, \'" + typeNew + "\');";
	}
	
	type = blur.match(/func_uior_valFormat_tips\(.*?\)[;]?/g);
	if (type) {
		blur = blur.replace(/func_uior_valFormat_tips\(.*?\)[;]?/, newFunc);
	} else {
		blur = blur + newFunc;
	}
	
	obj.setAttribute('onblur', blur);
}
//验证格式
function func_uior_valueFormat(obj){
	 ...
	 */
	var blur=obj.getAttribute('onblur');
	if (!blur || blur.indexOf("func_uior_valFormat_tips") === -1) {
		return "";
	}
	...
	if (type) {
		...
		try {
			//截取", 'idnum')"中的idnum。
			type = type.substring(type.indexOf(",") + 1, type.indexOf(")")).replace(/^\s*|\s$/g, "").replace(/^['"]|['"]$/g, "");
		} catch (err) {
		...
		if(type!='undefined'&&func_uior_trim(type)!=''){
			switch (type) {
				...
				case "officersNo":
					...
					msg = officersNo(obj);
					break;
				case "ratepositive"://校验利率数字不能小于等于零
					msg = rate_validate(obj);
					break;
				case "allidtype"://全部证件类型
					msg = checkAllIdType(obj,'idType');
				    break;
				case "legalIdType"://法人代表全部证件类型
					msg = checkAllIdType(obj,'legalIdType');
				    break;
				case "carFrameNo"://车架号
					msg = checkCarFrameNo(obj);
					break;
			    default:
			    	msg="字段验证规则错误";
			}
