var cusFinCapParmList = function(window,$){
	var _init = function(){
	...
	// 新增/配置
	var _showNewParm = function(type,codeColumn,codeName,obj,zcfzType){//type是1时是头部的新增，2是具体项的新增,3是具体项配置
		$globalObj = $(obj);
		globalType = type;
		if(reportType == "3"){//现金流方向
			$("#newParm-div").find("select[name=cashDirection]").parents(".rows:eq(0)").show();
		...
		if(type=="1"){
			$("#newParm-div").find("select[name=zcfzType]").parents(".rows:eq(0)").show();
			$("#newParm-div").find("input[name=parentCodeName]").parents(".rows:eq(0)").hide();
		}else{
			$("#newParm-div").find("select[name=zcfzType]").parents(".rows:eq(0)").hide();
			$("#newParm-div").find("input[name=parentCodeName]").parents(".rows:eq(0)").show();
		}
		if(type == "1" || type =="21" || type =="22"){
			$("#newParm-dgcctag input").val("");
			$("#newParm-dgcctag input[name=parentCodeName]").val(reportName);
			$("#newParm-dgcctag input[name=reportType]").val(reportType);
			$("#newParm-dgcctag select[name=zcfzType]").val(zcfzType);
	...
	var _ajaxInsertThis = function(obj){
		var parm = $("#newParm-div").find("select[name=zcfzType]").find('option:selected').text();
		$("#newParm-div").find("input[name=parentCodeName]").val(parm);
		...
		$.ajax({
			...
			beforeSend:function(){  
				...
				if(data.flag == "success"){
					 ...
					 var zcfzType = "";
					 var tdcss = "td-des";
					 if(data.cusFinParm.cnt!=null&&data.cusFinParm.cnt.length == 4){//
						 tdcss = "td-des1";
					 }else if(data.cusFinParm.cnt!=null&&data.cusFinParm.cnt.length == 6){
						 tdcss = "td-des2";
					 }
					 if(globalType=="1"){
						zcfzType = data.cusFinParm.zcfzType;
						 if(zcfzType == "1"){//资产类
							 htmlStr = '<td class="'+tdcss+'">'+data.cusFinParm.codeName+'</td>'
								+'<td class="td-caozuo"><span class="span-this" onclick="cusFinCapParmList.showNewParm(\'21\',\''+data.cusFinParm.codeColumn+'\',\''+data.cusFinParm.codeName+'\',this,\''+data.cusFinParm.zcfzType+'\');">新增</span>'
								+	'<span class="span-this" style="margin-left: 20px;" onclick="cusFinCapParmList.showNewParm(\'31\',\''+data.cusFinParm.codeColumn+'\',\''+data.cusFinParm.codeName+'\',this,\''+data.cusFinParm.zcfzType+'\');" >配置</span>'
								+'</td>';
						 }else{//负债类
							 htmlStr = '<td class="'+tdcss+'">'+data.cusFinParm.codeName+'</td>'
								+'<td class="td-caozuo"><span class="span-this" onclick="cusFinCapParmList.showNewParm(\'22\',\''+data.cusFinParm.codeColumn+'\',\''+data.cusFinParm.codeName+'\',this,\''+data.cusFinParm.zcfzType+'\');">新增</span>'
								+	'<span class="span-this" style="margin-left: 20px;" onclick="cusFinCapParmList.showNewParm(\'32\',\''+data.cusFinParm.codeColumn+'\',\''+data.cusFinParm.codeName+'\',this,\''+data.cusFinParm.zcfzType+'\');" >配置</span>'
								+'</td>';
						 }
					 }else if(globalType=="21" || globalType=="31"){
						htmlStr = '<td class="'+tdcss+'">'+data.cusFinParm.codeName+'</td>'
								+'<td class="td-caozuo"><span class="span-this" onclick="cusFinCapParmList.showNewParm(\'21\',\''+data.cusFinParm.codeColumn+'\',\''+data.cusFinParm.codeName+'\',this,\''+data.cusFinParm.zcfzType+'\');">新增</span>'
								+	'<span class="span-this" style="margin-left: 20px;" onclick="cusFinCapParmList.showNewParm(\'31\',\''+data.cusFinParm.codeColumn+'\',\''+data.cusFinParm.codeName+'\',this,\''+data.cusFinParm.zcfzType+'\');" >配置</span>'
								+'</td>';
					}else if(globalType=="22" || globalType=="32"){
						htmlStr = '<td class="'+tdcss+'">'+data.cusFinParm.codeName+'</td>'
								+'<td class="td-caozuo"><span class="span-this" onclick="cusFinCapParmList.showNewParm(\'22\',\''+data.cusFinParm.codeColumn+'\',\''+data.cusFinParm.codeName+'\',this,\''+data.cusFinParm.zcfzType+'\');">新增</span>'
								+	'<span class="span-this" style="margin-left: 20px;" onclick="cusFinCapParmList.showNewParm(\'32\',\''+data.cusFinParm.codeColumn+'\',\''+data.cusFinParm.codeName+'\',this,\''+data.cusFinParm.zcfzType+'\');" >配置</span>'
								+'</td>';
					...
					if(globalType=="1"){
						...
						if(zcfzType == "1"){
							$("#parmShow-table1 tbody").append(htmlStr);
						}else if(zcfzType == "2"){
							$("#parmShow-table2 tbody").append(htmlStr);
						}
					}else if(globalType=="21" || globalType=="22"){
						htmlStr = '<tr>' +htmlStr + '</tr>'; 
						$globalObj.parents("tr:eq(0)").after(htmlStr);
					}else if(globalType=="31" || globalType=="32"){
