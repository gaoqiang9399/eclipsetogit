 function func_getById(obj ,url){
	...
	c =	new RightForm(
		{actionUrl:url,formUrl:webPath+"/cusFinValid/updateAjax",title:"平衡校验"
...
function initCalculate(data){
	...
	if(modelList!=undefined){
		...
		$calculateDiv.find(".cBtn").click(function(){
	  		var filler = $("textarea[name='filler']");
	  		...
	  		if(text=="←"){
	  			...
	  			if(tempVals!==undefined&&tempTexts!==undefined){
	  				...
	  				filler.val(vals.substring(0,vals.length-1));
	  			...
	  			filler.val(filler.val()+$(this).text());
	  	...
	  	$select.find("option").dblclick(function(){
	  		var filler = $("textarea[name='filler']");
	  		var formName = $("textarea[name='formName']");
	  		filler.val(filler.val()+$(this).val());
	  		formName.val(formName.val()+$(this).text());
	  	});
...
function func_input(url){
	c =	new RightForm(
		{actionUrl:url,formUrl:webPath+"/cusFinValid/toInsertAjax",title:"平衡校验",btns:[{value:"继续",type:"button",onClick:"ajaxInput(this.form,ajaxInsertCallback)"}]}
	);
}
function ajaxInsertCallback(data){
	c =	new RightForm(
		{formFlag:true,dataForm:data,formUrl:webPath+"/cusFinValid/insertAjax",title:"平衡校验"
