;
var MfBusTour_List = function(window, $) {
	var _init = function() {
	...
	var _applyInsert = function() {
		top.openBigForm(webPath+"/mfBusTour/input","新增巡视记录", function(){
			updateTableData();
 		});	
	};
	...
	var _selectName = function(callback){
		...
		dialog({
			id:"selectNameDialog",
			title:'选择职员',
			url: webPath+"/mfOaArchivesBase/getNameSelect?opSts=3",
			width:800,
			height: tmpheight,
			backdropOpacity:0,
			onshow:function(){
	...
	var _changeBaseInfo = function(data){
		$.ajax({
			url:webPath+"/mfBusInstitutions/getByIdAjax",
			...
			success:function(data){
				if(data.flag == "success"){
					...
					$("input[name = 'sex']").val(data.mfOaArchivesBase.sex);
					$("input[name = 'sexShow']").val(data.mfOaArchivesBase.sexShow);
