;
var MfKindDocConfig=function(window, $){
    //初始化产品影像配置信息
    ...
    var getPrdctDocConfigHtml = function (data) {
        ...
        htmlStr = htmlStr + '<div class="content-div docConfig special-config"><div class="sub-content-div padding_left_15">'
            ...
            + '<button  onclick="MfSysKindConfig.addBusNode(\'' + mfSysKind.busModel + '\',\'doc\');" class="margin_left_15  btn add-node pointer">节点定制</button>'
            + '</div>'
    ...
    var getPrdctDocContentHtml = function (nodeList, nodeConfigMap, mfSysKind) {
        ...
        $.each(nodeList, function (i, kindNode) {
            /*htmlStr =htmlStr+'<div1 class="sub-content-div padding_left_5" style="position:relative;">'
            +'<div class="horizontal-bar"><i class="i i-yuan"></i></div>'

            +'<div class="sub-title">'
            +'<span class="font_size_14"  id='+kindNode.nodeNo+'>'+kindNode.nodeName+'</span>'
            +'</div>'

            +'<div2 class="sub-content">'
            +'<div3 class="item-div docNode'+kindNode.nodeNo+'" data-scno='+kindNode.nodeNo+'>'
            +'<div4 class="item-content padding_bottom_20">'
            +'<div5 class="main-content-div">';
            var docList = nodeConfigMap[kindNode.nodeNo]["docList"];
            var  nodeDocContenHtml  =    getNodeDocContentHtml(docList,kindNode.nodeNo);
            htmlStr =htmlStr + nodeDocContenHtml;
            htmlStr =htmlStr +'</div5>  </div4>  </div3>   </div2>   </div1>';*/
            ...
            if (isExistDoc == "1") {
                ...
                $button.attr("onclick", "MfSysKindConfig.addBusNode('" + mfSysKind.busModel + "','doc','" + nodeNo + "','" + mainApprove + "');");
    ...
    var getNodeDocContentHtml = function (docList, scNo) {
        ...
        $.each(docList, function (i, docConfig) {
            ...
            if (docConfig.ifMustInput == "1") {
                spanStr=spanStr+'<span style="background: #85B7EE;"><input class="pointer" onclick="MfKindDocConfig.changeIsMust(this);" type="checkbox" checked="true" >必填</span>'
            } else {
                spanStr=spanStr+'<span style="background: #85B7EE;"><input class="pointer" onclick="MfKindDocConfig.changeIsMust(this);" type="checkbox">必填</span>'
            }
            htmlStr = htmlStr + '<div class="block-item" data-doctype="' + docConfig.docType + '" data-docno="' + docConfig.docSplitNo + '" data-formno="' + docConfig.formId + '" data-ismust="' + docConfig.ifMustInput + '">'
                ...
                + '</div>'
                + '<i class="i i-x5 item-del pointer" onclick="MfKindDocConfig.deleteDocTypeItem(this);"></i>'
                + '</div>';
        });
        //添加按钮
        htmlStr = htmlStr + '<div class="add-item-div" onclick="MfKindDocConfig.getDocTypeData(this);">'
            + '<div class="add-item">'
            + '<div class="item">'
            + '</div>'
            + '</div>'
