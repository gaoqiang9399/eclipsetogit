...
<head >
	...
	<!-- 同盾认证报告 -->
		...
		var docParm = "query="+queryFile+"&cusNo="+cusNo+"&relNo="+cusNo+"&appId="+appId+"&pactId="+pactId+"&pactModelNo="+pactModelNo+"&fincId="+fincId;//查询文档信息的url的参数
        ...
        var extensionApplyId = '${extensionApplyId}';// 该借据下最新一条展期的申请ID
        var extenSts = '${extenSts}'; // 实时展期状态 展期业务状态0未展期1展期中2展期完成
        var extenApprove = '${extenApprove}';//展期审批状态
			...
			<div class="col-md-8 column block-left">
				<div class="bg-white block-left-div">
					...
					<c:forEach items="${ dataMap.headView}" var="item">
							<c:if test="${isShowBtn == null}">
								<jsp:include page="${item.blockUrl }"   flush="true"></jsp:include>
							</c:if>
							<c:if test="${!fn:contains(item.blockUrl, 'pub_finc_operat_buttons.jsp') and isShowBtn != null}">
								<jsp:include page="${item.blockUrl }"   flush="true"></jsp:include>
							</c:if>
					</c:forEach>
