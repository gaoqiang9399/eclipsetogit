;
var MfLitigationExpenseApply_Insert = function(window, $) {
	var _init = function() {
	...
	function submit(obj){
		var dataParam = JSON.stringify($(obj).serializeArray());
		jQuery.ajax({
			url:"/mfLitigationExpenseApply/applyUpdate",
			data:{ajaxData:dataParam,litigationId:relNo,applyStatus:applyStatus},
			type:"POST",
			dataType:"json",
			beforeSend:function(){ 
				LoadingAnimate.start();
