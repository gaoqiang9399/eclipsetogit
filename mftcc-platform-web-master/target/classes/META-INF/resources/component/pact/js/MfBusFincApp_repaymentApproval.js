;
var MfBusFincApp_repaymentApproval  = function(window, $) {
    var _init = function () {
        //滚动条
    ...
    var _selectFincBack = function () {
       ...
       var  fincNo = $("input[name='fincNo']").val();
        ...
        $.ajax({
            ...
            data:{fincNo:fincNo,repayAppId:repayAppId},
            dataType:'json',
            type:'post',
            success : function(data) {
                ...
                $.each(fincHtmlList, function (index, obj) {
                    ...
                    $("#fincList").css('display','none');
                    $("#"+obj.mfBusFincApp.fincId+"List").append(obj.htmlStr);
                });
                $.each($(".ls_list"), function (index, obj) {
                    if(repayAppId != null && repayAppId != ""){
                        $(obj).find( "tbody tr:eq(1) td input").attr("readonly","readonly");
                    }
    ...
    var _batchRepayment = function (formObj) {
        ...
        if(flag){
            ...
            $.ajax({
                url:url,
                data:{ajaxData:dataForm,ajaxList:jsonString,repayId:repayId},
                type:"post",
                dataType:"json",
                success:function(data){
                    LoadingAnimate.stop();
    ...
    //还款确认
    var _repaymentConfirm=function (formObj) {
        var dataForm = JSON.stringify($(formObj).serializeArray());
        var submitExplain=$("input[name='submitExplain']").val();
        if(submitExplain==null || submitExplain==''){
            alert('说明不能为空',0);
            return;
        }
        LoadingAnimate.start();
        $.ajax({
            url: webPath+"/mfRepayApply/repaymentConfirm",
            data:{ajaxData:dataForm,repayAppId:repayAppId},
            type:"post",
            dataType:"json",
            success:function(data){
                LoadingAnimate.stop();
    ...
    return {
        ...
        repaymentConfirm:_repaymentConfirm
    };
}(window, jQuery);