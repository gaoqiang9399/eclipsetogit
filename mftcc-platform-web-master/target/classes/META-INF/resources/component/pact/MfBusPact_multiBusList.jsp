...
<html>
	...
	var mfBusFincAppProjectListSize='${mfBusFincAppProjectListSize}';
	$(function(){
		//处理暂无数据的情况
		if(mfBusFincAppListSize == "0" && mfBusFincAppProjectListSize == "0"){
	...
	function enterClick(obj,url){
		...
		}else if(busEntrance=="pact"){
			$(top.window.document).find(".pt-page-current").find("iframe").eq(0).attr("src",url+"&busEntrance=pact");
			myclose();
		}	
			else{
			$(top.window.document).find(".pt-page-current").find("iframe").eq(0).attr("src",url+"&busEntrance=2");
			myclose();
		}
...
<body class="bg-white">
 	<div class="container scroll-content">
 		 <div class="row clearfix">
			 <c:if test="${mfBusFincAppList != null and fn:length(mfBusFincAppList) > 0}">
				 <div class="arch_sort">
			 ...
			 <c:if test="${mfBusFincAppProjectList != null and fn:length(mfBusFincAppProjectList) > 0}">
				 <div class="arch_sort">
