...
<script type="text/javascript">
	$(function(){
		...
		var repayTypelist = dataMap.repayTypelist;
		var html = "<table><tbody>";
		var ind = 0;
		$.each(repayTypelist,function(i,repayType){
			ind++;
			if(ind == 1){
			...
			html = html + '<li class="risk_class_item item_false" data-kindno="'+repayType.optCode+'" data-kindname="'+repayType.optName+'" title="'+repayType.optName+'" onclick="getNotSelectedItem(this);">'
					+'<span class="checkbox_false"><i class="i i-gouxuan"></i></span> '
					+'<label class="item_key"></label>'
					+'<lable style="padding-left: 5px;">'+repayType.optName+'</lable>'
					+'</li>';
			if(ind == 3){
				html = html + "</td></tr>";
				ind =0;
	...
	function confirm(){
		var repayTypeNo="";
		var repayTypeDes="";
		var repayType = new Object();
		var length = $("li.risk_class_item.item_true").length;
		$("li.risk_class_item.item_true").each(function(i,objThis){
			if(i == length-1){
				repayTypeNo = repayTypeNo + $(objThis).data("kindno");
				repayTypeDes = repayTypeDes + $(objThis).data("kindname");
			}else{
				repayTypeNo = repayTypeNo + $(objThis).data("kindno")+"@";
				repayTypeDes = repayTypeDes + $(objThis).data("kindname")+"@";
			}
		});
		repayType.repayTypeNo = repayTypeNo;
		repayType.repayTypeDes = repayTypeDes;
		parent.dialog.get("repayTypeMutiDialog").close(repayType);
	};
</script>
</head>
