;
var MfBusTemplate_fileList = function(window, $) {
    var _init = function(){
        // debugger;
...
var MfBusTemplate_fileList = function(window, $) {
	...
	//打包下载,获取参数
	var _downloadTemplateZip = function(appId,cusNo,templateBizConfigIds){
		var ajaxData =templateBizConfigIds;
	    try{ 
	    	if(templateBizConfigIds==""){
	    		alert(top.getMessage("CONFIRM_OPERATION_REASON",{
	    			...
	    			var url = webPath+"/docUpload/getZipFileDownloadForTemplet?appId="+appId+"&cusNo="+cusNo+"&ajaxData="+ajaxData;
	    			var elemIF = document.createElement("iframe");   
	    			elemIF.src = url;   
	    			elemIF.style.display = "none";   
	    			document.body.appendChild(elemIF);   
	...
	return {
		...
		downloadTemplateZip : _downloadTemplateZip
	};
}(window, jQuery);
*/
