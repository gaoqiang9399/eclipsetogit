...
.vch-date {
	...
	cursor: auto;
	background: url(../voucher/images/datepicker_icon.png) right 0 no-repeat
		#FFF;
}
				...
				<c:forEach items="${dataMap.list }" var="items">
					<c:if test='${items.pcode!="10001" }'>
						<div class="row set_item" id="code${items.pcode }"  data-val="${items.pcode }">
							...
							<div class="div2">
							<c:forEach items="${items.parmList }" var="pl" varStatus="st">
									<input type="radio" name="pcode${items.pcode }" pcode="${items.pcode }" value="${pl[0]}" <c:if test="${pl[0]==items.pvalue}">checked</c:if> onclick="updateParamValue(this)">
									<label class="span2" for="pcode${items.pcode }">${pl[1] }</label>
									<br>
							</c:forEach>
							</div>
						</div>
...
<script type="text/javascript">
	...
	$(function() {
        if(comDits !=''){
            ...
            for(var i=0; i<4; i++){
                if (i==0){
                    opt.add(new Option(comDits + '-2-2（默认）' , i));
                } else {
                    opt.add(new Option(comDits + '-' + (i + 2) + '-' + (i + 2), i));
                }
            }
        }
	...
	function jiTiType(){
		...
		//window.parent.openBigForm("CwSysParam/getJiTiType.action","计提方式设置",'90','90');
